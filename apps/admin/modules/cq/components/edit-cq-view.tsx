...
import { useUpdateCq } from "../services/use-cq"
...
import type { CqItem } from "./cq-table"
...
interface EditCqViewProps {
  cq: CqItem
}

export function EditCqView({ cq }: EditCqViewProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      context: cq.context || "",
      referenceText: cq.reference?.join(", ") || "",
      questionA: cq.questionA,
      questionB: cq.questionB,
      questionC: cq.questionC,
      questionD: cq.questionD || "",
      answer: {
        answerA: cq.answer?.answerA || "",
        answerB: cq.answer?.answerB || "",
        answerC: cq.answer?.answerC || "",
        answerD: cq.answer?.answerD || "",
        explanation: cq.answer?.explanation || "",
      },
      attachments: cq.attachments.map((att) => ({
        id: att.id,
        url: att.url,
        type: att.type,
        caption: att.caption || "",
  ...
  const onSubmit = async (data: EditCqFormData) => {
    ...
    try {
      ...
      const msg = err.message || "Failed to update CQ"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
    ...
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-0">
      ...
      <div className="mb-6">
        <h2 className="font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
          Edit Creative Question (CQ)
        </h2>
        <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed mt-1">
          Update creative question details, stem descriptions, parts, and answer key details.
        </p>
      </div>
