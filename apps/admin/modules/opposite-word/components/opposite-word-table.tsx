...
export interface OppositeWordItem {
  id: string
  word: string
  oppositeWord?: string | null
...
interface OppositeWordTableProps {
  items: OppositeWordItem[]
  isLoading: boolean
  isError: boolean
  onDelete: (id: string, wordSnippet: string) => void
...
export function OppositeWordTable({
  items,
  isLoading,
  isError,
  onDelete,
      ...
      {isLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading Opposite Words...</span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-error rounded-xl border border-error/30 bg-error-container/20">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">
              Error loading opposite words. Please try refreshing.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="font-body-md text-sm text-outline max-w-md">
              Try adjusting filters or add a new opposite word to the bank.
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                  ...
                  <div className="flex items-start gap-3 md:gap-4 flex-1 min-w-0 w-full relative md:static">
                    ...
                    <div className="flex-1 space-y-4 min-w-0">
                      ...
                      {/* Word & Opposite Word Content Box */}
                      <div className="rounded-xl border border-secondary/20 bg-secondary-container/10 p-4 text-sm text-on-surface leading-relaxed space-y-3">
                        {/* Primary Word */}
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-bold text-outline text-xs uppercase tracking-wider">মূল শব্দ:</span>
                          <span className={cn(
                            "whitespace-pre-wrap font-bold text-lg text-primary",
                            /[\u0980-\u09FF]/.test(item.word) && "font-solaiman text-xl"
                          )}>
                            <RenderMath text={item.word} isMath={true} />
                          </span>
                        </div>

                        {/* Opposite Word */}
                        {item.oppositeWord && (
                          <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-secondary/15">
                            <span className="font-bold text-outline text-xs uppercase tracking-wider">বিপরীত শব্দ:</span>
                            <span className={cn(
                              "whitespace-pre-wrap font-bold text-base text-emerald-700",
                              /[\u0980-\u09FF]/.test(item.oppositeWord) && "font-solaiman text-lg"
                            )}>
                              <RenderMath text={item.oppositeWord} isMath={true} />
                            </span>
                          </div>
                        )}

                        {/* Alternative Opposite Words */}
                        {Array.isArray(item.oppositeWords) && item.oppositeWords.length > 0 && (
                          <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-secondary/15">
                            <span className="font-bold text-outline text-xs uppercase tracking-wider">বিকল্প বিপরীত শব্দ:</span>
                            <div className="flex flex-wrap gap-1.5">
                              {item.oppositeWords.map((altWord, altIdx) => (
                                <span
                                  key={altIdx}
                                  className={cn(
                                    "px-2 py-0.5 bg-white border border-secondary/20 rounded text-xs font-semibold text-on-surface",
                                    /[\u0980-\u09FF]/.test(altWord) && "font-solaiman text-sm"
                                  )}
                                >
                                  {altWord}
                                </span>
                  ...
                  <div className="flex md:flex-col justify-end items-center gap-2 shrink-0 border-t md:border-t-0 border-outline-variant/40 pt-3 md:pt-0 w-full md:w-auto">
                    <Link
                      href={`/opposite-words/${item.id}/edit`}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border border-outline-variant bg-surface-container-low rounded-xl p-4">
          <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-start">
            <p className="font-body-md text-xs sm:text-sm text-on-surface-variant">
              Showing <span className="font-bold">{displayStart}-{displayEnd}</span> of <span className="font-bold">{totalItems}</span> opposite words
            </p>
            {onLimitChange && (
              <div className="flex items-center gap-2">
