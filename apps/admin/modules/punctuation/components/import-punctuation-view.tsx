...
interface EditablePunctuationCardProps {
  index: number
  item: any
...
function EditablePunctuationCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditablePunctuationCardProps) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant/40 bg-surface-container-low/60 px-5 py-3.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            size="icon"
            onClick={onDelete}
            className="size-8 text-on-surface-variant hover:text-destructive hover:bg-destructive/10 rounded-lg cursor-pointer"
      ...
      <CardContent className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
          ...
          <EditableField
            ...
            value={String(item.popularityCount ?? 0)}
            onSave={(newVal) => onChange({ ...item, popularityCount: Number(newVal) || 0 })}
            placeholder="0"
          />
        </div>
      </CardContent>
...
export function ImportPunctuationView() {
  const router = useRouter()
  const importMutation = useImportPunctuation()
  ...
  const [errorContext, setErrorContext] = useState<{
    line: number
    col: number
    linesContext: { num: number; content: string; isError: boolean }[]
  } | null>(null)
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        } else if (Array.isArray(parsed.punctuation)) {
          itemsArray = parsed.punctuation
        } else {
          itemsArray = [parsed]
        }
      } else {
        setParseError("JSON root must be an array of question objects.")
        setParsedItems([])
        return
      }

      if (itemsArray.length === 0) {
        setParseError("The JSON array contains no question items.")
        setParsedItems([])
        return
      }
      ...
      const errorPos = findJsonErrorPosition(rawText, msg)
      setErrorContext(errorPos)
      setParsedItems([])
    }
  }
  ...
  const handleUpdateParsedItem = (idx: number, updatedItem: any) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ punctuations: payload })
      toast.success(`Successfully imported ${res.count} Punctuation questions!`)
      setTimeout(() => {
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Punctuation & Capitalization from JSON
          </h2>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Question items will be assigned subject from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          {parsedItems.length > 0 && (
            <div className="border-t border-outline-variant/30 pt-6 space-y-6">
              ...
              {/* Grid of Punctuation Preview Cards */}
              <div className="space-y-6">
                {parsedItems.map((item, idx) => (
                  <EditablePunctuationCard
                    key={idx}
                    index={idx}
                    item={item}
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/punctuation")}
