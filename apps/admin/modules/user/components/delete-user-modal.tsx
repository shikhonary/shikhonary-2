...
import { useDeleteUser } from "../services/use-user"
...
export function DeleteUserModal() {
  const { isOpen, userId, userName, closeModal } =
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(
        `User "${userName || "User"}" deleted successfully.`
      )
      closeModal()
    } catch (err: any) {
      ...
      >
        ...
        <DialogHeader className="pt-1">
          <DialogDescription className="font-body-md text-sm leading-relaxed text-on-surface-variant">
            ...
            ? All associated session records, account links, and role bindings will be permanently deleted.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-700">
          ...
          <p className="leading-snug">
            This action cannot be undone. The user will be logged out and will lose all access.
          </p>
        </div>
