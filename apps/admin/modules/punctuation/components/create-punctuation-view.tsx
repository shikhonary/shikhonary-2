...
const createPunctuationFormSchema = z.object({
...
type CreatePunctuationFormData = z.infer<typeof createPunctuationFormSchema>

export function CreatePunctuationView() {
  const router = useRouter()
  const createMutation = useCreatePunctuation()
  ...
  } = useForm<CreatePunctuationFormData>({
    resolver: zodResolver(createPunctuationFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      content: "",
  ...
  const onSubmit = async (data: CreatePunctuationFormData) => {
    ...
    try {
      ...
      toast.success("Punctuation question created successfully.")
      setTimeout(() => {
        router.push("/punctuation")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to create Punctuation question"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary flex items-center gap-2">
            ...
            New Punctuation Question
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Add a new punctuation and capitalization passage with academic properties and board references.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="border-b border-outline-variant/40 bg-surface-container-lowest p-4 sm:p-8 flex flex-row items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="font-body-sm text-xs sm:text-sm text-on-surface-variant">
              Fill in academic details and passage content for punctuation practice.
            </p>
          </div>
        </CardHeader>
