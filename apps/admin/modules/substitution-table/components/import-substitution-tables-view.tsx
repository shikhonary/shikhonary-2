...
const sampleJsonTemplate = `[
  {
    ...
    "columnC": [
      ...
      "a man to distinguish between right and wrong.",
      "in life without education.",
      "a vital role in developing the nation.",
      "proper steps to eradicate illiteracy."
    ],
...
function EditableCard({
  ...
  const handleSaveColumn = (colKey: "columnA" | "columnB" | "columnC", val: string) => {
    const rows = val
      .split("\n")
      .map((r) => r.trim())
      .filter(Boolean)
  ...
  const colAString = Array.isArray(item.columnA) ? item.columnA.join("\n") : ""
  const colBString = Array.isArray(item.columnB) ? item.columnB.join("\n") : ""
  const colCString = Array.isArray(item.columnC) ? item.columnC.join("\n") : ""
    ...
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant/40 bg-surface-container-low/60 px-5 py-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-outline flex items-center gap-1.5">
      ...
      <div className="p-5 space-y-4">
        {/* Rendered Live Table Grid Preview */}
        <div>
          <span className="font-label-sm text-xs font-bold uppercase tracking-wider text-outline block mb-1">
            Table Preview
          </span>
          <SubstitutionTableGrid
            columnA={Array.isArray(item.columnA) ? item.columnA : []}
            columnB={Array.isArray(item.columnB) ? item.columnB : []}
            columnC={Array.isArray(item.columnC) ? item.columnC : []}
          />
        </div>

        {/* 3 Column Editors */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2 border-t border-outline-variant/30">
          <EditableField
            label="Column A (One row per line)"
            value={colAString}
            multiline
            placeholder="(a) Clause 1&#10;(b) Clause 2..."
            onSave={(val) => handleSaveColumn("columnA", val)}
          />
          <EditableField
            label="Column B (One row per line)"
            value={colBString}
            multiline
            placeholder="Verb 1&#10;Verb 2..."
            onSave={(val) => handleSaveColumn("columnB", val)}
          />
          <EditableField
            label="Column C (One row per line)"
            value={colCString}
            multiline
            placeholder="Extension 1&#10;Extension 2..."
            onSave={(val) => handleSaveColumn("columnC", val)}
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-outline-variant/30 pt-3">
          <EditableField
            ...
            placeholder="e.g. Dhaka Board 2024, Rajshahi Board..."
...
export function ImportSubstitutionTablesView() {
  const router = useRouter()
  const importMutation = useImportSubstitutionTables()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      if (repaired && repaired !== text) {
        try {
          ...
          toast.success("Auto-repaired JSON syntax!")
        } catch {
          // Ignore
        }
      }
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        const colA = Array.isArray(item.columnA)
          ? item.columnA
          : typeof item.columnA === "string"
          ? item.columnA.split("\n").map((s: string) => s.trim()).filter(Boolean)
          : []

        const colB = Array.isArray(item.columnB)
          ? item.columnB
          : typeof item.columnB === "string"
          ? item.columnB.split("\n").map((s: string) => s.trim()).filter(Boolean)
          : []

        const colC = Array.isArray(item.columnC)
          ? item.columnC
          : typeof item.columnC === "string"
          ? item.columnC.split("\n").map((s: string) => s.trim()).filter(Boolean)
          : []

        if (colA.length === 0 || colB.length === 0 || colC.length === 0) {
          errors.push(`Item #${itemNum}: Missing columnA, columnB, or columnC row items.`)
        }
        if (!subjectId) {
      ...
      setParseError(
        `Validation Error: ${err.message || "Could not validate items."}`
      )
    }
  }
  ...
  const handleDeleteParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.info(`Removed table #${idx + 1} from import list.`)
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.success(`Duplicated table #${idx + 1}.`)
  }

  const handleAddNewQuestionCard = () => {
    const newItem = {
      columnA: ["(a) Education", "(b) It", "(c) Nobody", "(d) An educated person", "(e) We"],
      columnB: ["is", "enables", "can prosper", "plays", "should take"],
      columnC: [
        "the backbone of a nation.",
        "a man to distinguish between right and wrong.",
        "in life without education.",
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ items: payload })
      toast.success(`Successfully imported ${res.importedCount} substitution tables!`)
      setTimeout(() => {
        router.push("/substitution-tables")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import substitution tables.")
    }
  }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Substitution Tables from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload 3-column meaningful sentence substitution tables or edit cards before importing.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Table items will inherit this selection.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t border-outline-variant/30">
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="cursor-pointer">
                  <Button
                    ...
                    size="xs"
                    asChild
                    className="text-xs font-bold border-outline-variant/60 cursor-pointer"
                  >
                    <span>
            ...
            <Textarea
              ...
              placeholder="Paste JSON array containing columnA, columnB, columnC here..."
              className="font-mono text-xs leading-relaxed bg-surface-container-lowest/50 border-outline-variant/60 focus:border-primary"
            />
            ...
            {parseError && (
              <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 space-y-2">
                ...
                {errorContext && errorContext.linesContext && (
                  <div className="rounded-lg bg-surface-container-lowest p-3 font-mono text-xs border border-destructive/20 overflow-x-auto">
                    {errorContext.linesContext.map((l) => (
                      <div
                        key={l.num}
                        className={cn(
                          "py-0.5 px-2 rounded",
                          l.isError && "bg-destructive/10 text-destructive font-bold"
                        )}
                      >
                        <span className="inline-block w-8 text-outline select-none">
                          {l.num}:
                        </span>
                        <span>{l.content}</span>
                      </div>
      ...
      {/* Visual Item Cards Preview & Edit */}
      {parsedItems.length > 0 && !parseError && (
        <div className="mt-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="font-headline text-lg font-bold text-on-surface">
                Parsed Substitution Tables ({parsedItems.length})
              </h3>
              <p className="text-xs text-on-surface-variant">
                Preview or double-click to edit any card before importing.
              </p>
            </div>
            <Button
              type="button"
          ...
          <div className="flex items-center justify-end gap-3 pt-6 border-t border-outline-variant/40">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/substitution-tables")}
            ...
            <Button
              type="button"
              disabled={importMutation.isPending || parsedItems.length === 0 || !selectedSubjectId}
            ...
            >
              {importMutation.isPending
                ? "Importing..."
                : `Import ${parsedItems.length} Table(s)`}
            </Button>
          </div>
        </div>
      )}
