...
import { MoreVertical, Pen, Trash, User as UserIcon, CheckCircle2, Clock, ShieldAlert, Shield } from "lucide-react"
...
interface UserDataTableProps {
  ...
  onChangeRole?: (item: UserItem) => void
      ...
      {isLoading ? (
        <div className="p-4 md:p-12">
          ...
          <div className="hidden md:flex items-center justify-center p-8 text-on-surface-variant">
            ...
            <span className="ml-3 font-body-md">Loading users...</span>
          </div>
        </div>
      ) : isError ? (
        <div className="p-8 text-center text-error">
          <span className="material-symbols-outlined text-4xl">error</span>
          <p className="mt-2 font-body-md font-medium">Failed to load user directory.</p>
        </div>
      ) : items.length === 0 ? (
        <div className="p-8 sm:p-12 text-center">
          ...
          <p className="mt-1 font-body-md text-sm text-on-surface-variant">
            Get started by inviting or creating your first institutional user.
          </p>
          <div className="mt-6">
            <Button
              asChild
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <Avatar className="w-10 h-10 size-10 rounded-full border border-outline-variant/30 shrink-0">
                      {item.image && <AvatarImage src={item.image} alt={item.name ?? ""} className="object-cover" />}
                      <AvatarFallback className="w-full h-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm">
                        {getInitials(item.name, item.email)}
                      </AvatarFallback>
                    </Avatar>
                    <div className="min-w-0 flex-1">
                      <h4 className="font-headline-md text-sm font-extrabold text-on-surface truncate">
                        {item.name || "Unnamed User"}
                      </h4>
                      <p className="text-xs text-outline font-mono truncate">{item.email || "No Email"}</p>
                      {item.phoneNumber && (
                        <p className="text-[11px] text-outline/80 font-mono mt-0.5">{item.phoneNumber}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="bg-white border border-outline-variant shadow-md rounded-xl p-1.5 min-w-[140px]">
                      ...
                      <DropdownMenuItem
                        variant="destructive"
                        onClick={() => onDelete(item.id, item.name || item.email || "User")}
                        disabled={isDeleting}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {item.roles.length > 0 ? (
                      item.roles.map((role) => (
                        <Badge
                          key={role.id}
                          className={`rounded-full px-2 py-0.5 font-label-sm text-[9px] font-bold uppercase border shadow-none ${getRoleBadgeClass(
                            role.name
                          )}`}
                        >
                          {role.name}
                      ...
                      <Badge className="bg-surface-variant text-on-surface-variant rounded-full px-2 py-0.5 font-label-sm text-[9px] font-bold uppercase border-0">
                        No Roles
                      </Badge>
                    )}
                  </div>
                  <div>
                    {item.emailVerified || item.phoneNumberVerified ? (
                      <Badge className="bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 shadow-none px-2 py-0.5 text-[9px] uppercase font-bold rounded-full">
                        Verified
                      </Badge>
                    ) : (
                      <Badge className="bg-amber-500/10 text-amber-600 border border-amber-500/20 shadow-none px-2 py-0.5 text-[9px] uppercase font-bold rounded-full">
                        Pending
                      </Badge>
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between border-t border-outline-variant/20 pt-2.5 text-[11px] text-outline">
                  ...
                  <span className="text-[10px] text-outline/80 font-mono">
                    ...
                    <TableCell className="py-4 group-hover:py-5 px-6 transition-all duration-200 ease-in-out">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-headline-md text-sm font-bold text-on-surface truncate">
                            {item.name || "Unnamed User"}
                          </p>
                          <p className="text-xs text-outline font-mono truncate">{item.email || "No Email Address"}</p>
                          {item.phoneNumber && (
                            <p className="text-[11px] text-outline font-mono mt-0.5">{item.phoneNumber}</p>
                          )}
                        </div>
                      </div>
                    </TableCell>
                      ...
                      <div className="flex flex-wrap gap-1.5 max-w-[240px]">
                        {item.roles.length > 0 ? (
                          item.roles.map((role) => (
                            <Badge
                              key={role.id}
                              className={`rounded-full px-2.5 py-0.5 font-label-sm text-[10px] font-bold uppercase border shadow-none ${getRoleBadgeClass(
                                role.name
                              )}`}
                            >
                              {role.name}
                          ...
                          <Badge className="bg-surface-variant text-on-surface-variant rounded-full px-2.5 py-0.5 font-label-sm text-[10px] font-bold uppercase border shadow-none">
                            No Roles
                          </Badge>
                        )}
                      </div>
                    ...
                    <TableCell className="py-4 group-hover:py-5 px-6 transition-all duration-200 ease-in-out">
                      {item.emailVerified || item.phoneNumberVerified ? (
                        <div className="flex items-center gap-1.5 text-emerald-600">
                          ...
                          <span className="font-body-md text-xs font-semibold uppercase">Verified</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5 text-amber-600">
                          <Clock className="h-4.5 w-4.5 shrink-0" />
                          <span className="font-body-md text-xs font-semibold uppercase">Pending</span>
                        </div>
                      )}
                    </TableCell>
