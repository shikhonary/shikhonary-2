...
import { useImportPoems, useAcademicClassesForSelection, useSubjectsForSelection } from "../services/use-poem"
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <span className={cn(
              ...
              <RenderMath text={value ? value.replace(/\\n/g, "\n") : placeholder} isMath={isMath} />
            </span>
            <button
              type="button"
              onClick={(e) => {
...
interface EditablePoemCardProps {
  index: number
  item: any
...
function EditablePoemCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditablePoemCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        {/* Title / Text */}
        <EditableField
          label="📜 পদ্যাংশ / কবিতার স্তবক"
          value={item.title || ""}
          multiline
          placeholder="Enter poem verses..."
...
export function ImportPoemView() {
  const router = useRouter()
  const importMutation = useImportPoems()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        const titleText = item.title || item.poem || item.text || item.name || ""

        if (!titleText || typeof titleText !== "string" || !titleText.trim()) {
          errors.push(`Item #${itemNum}: Missing or invalid poem text/title.`)
        }
        if (!subjectId) {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Poem items."}`)
    }
  }
  ...
  const handleDeleteParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.info(`Removed poem #${idx + 1} from import list.`)
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.success(`Duplicated poem #${idx + 1}.`)
  }

  const handleAddNewQuestionCard = () => {
    const newPoem = {
      title: "নতুন কবিতার স্তবক...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
    const next = [...parsedItems, newPoem]
    syncParsedItemsToText(next)
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ poems: payload })
      toast.success(`Successfully imported ${res.importedCount} Poems!`)
      setTimeout(() => {
        router.push("/poems")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import poems.")
    }
  }
    ...
    <div className="w-full max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload poems or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Poem items will be assigned subject from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder='Paste JSON array here e.g. [{"title": "...", "reference": ["..."]}]'
              className="w-full rounded-xl border border-outline-variant bg-white p-4 font-mono text-xs text-on-surface transition-all focus:border-primary focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20 focus-visible:outline-hidden disabled:opacity-50 disabled:cursor-not-allowed leading-relaxed"
            />
          </div>

          {/* Diagnostic Error Display */}
          {parseError && (
            <div className="rounded-xl border border-error/40 bg-error-container/20 p-4 space-y-3">
              <div className="flex items-start gap-2.5 text-error">
                <AlertTriangleIcon className="size-5 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <p className="font-bold text-sm">JSON Parsing / Validation Error</p>
                  <p className="font-mono text-xs">{parseError}</p>
                </div>
              </div>

              {errorContext && (
                <div className="rounded-lg bg-red-950 p-3 font-mono text-xs text-red-200 overflow-x-auto space-y-1">
                  <p className="text-[11px] text-red-400 font-bold mb-1">
                    Error Location: Line {errorContext.line}, Column {errorContext.col}
                  </p>
                  {errorContext.linesContext.map((l: any) => (
                    <div
                      ...
                      className={cn(
                        ...
                        l.isError ? "bg-red-900/80 text-white font-bold" : "text-red-300/70"
                      )}
                    >
                      <span className="w-8 text-right text-red-400/60 select-none">{l.num}</span>
      ...
      <div className="mt-8 flex items-center justify-end gap-3">
        ...
        <Button
          ...
          className="rounded-xl bg-primary hover:bg-primary/90 text-white font-bold px-6 h-11 gap-2 shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {importMutation.isPending ? (
            <span>Importing Poems...</span>
          ) : (
            <span>Import {parsedItems.length} Poems</span>
          )}
        </Button>
      </div>
    </div>
