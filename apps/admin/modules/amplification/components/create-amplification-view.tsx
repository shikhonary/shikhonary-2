...
import { useCreateAmplification } from "../services/use-amplification"
...
const createAmplificationFormSchema = z.object({
...
type CreateAmplificationFormData = z.infer<typeof createAmplificationFormSchema>

export function CreateAmplificationView() {
  const router = useRouter()
  const createMutation = useCreateAmplification()
  ...
  } = useForm<CreateAmplificationFormData>({
    resolver: zodResolver(createAmplificationFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: CreateAmplificationFormData) => {
    ...
    try {
      ...
      toast.success("Amplification created successfully.")
      setTimeout(() => {
        router.push("/amplifications")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to create Amplification"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Add a new amplification proverb/quotation to the question bank, defining its thought text, academic properties, and references.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="border-b border-outline-variant/40 bg-surface-container-lowest p-4 sm:p-8 flex flex-row items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="text-[11px] sm:text-xs font-body-md text-on-surface-variant mt-0.5">
              Configure academic properties, write the thought proverb, and configure references.
            </p>
          </div>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="title"
                disabled={isSubmitting}
                placeholder="Enter the quotation, proverb, or thought line to expand (e.g. লোভে পাপ, পাপে মৃত্যু). Supports LaTeX..."
                {...register("title")}
                rows={4}
            ...
            <div className="flex justify-end gap-4 border-t border-outline-variant/30 pt-6">
              ...
              >
                {isSubmitting ? "Creating..." : "Save Amplification"}
              </Button>
            </div>
          </form>
        </CardContent>
