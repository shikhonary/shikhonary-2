...
import { useImportLetters, useAcademicClassesForSelection, useSubjectsForSelection } from "../services/use-letter"
...
const sampleJsonTemplate = `[
  ...
  {
    "title": "Write a letter to your friend congratulating him on his brilliant success in the SSC examination.",
...
interface EditableLetterCardProps {
  index: number
  item: any
...
function EditableLetterCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableLetterCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2 border-l border-outline-variant/60 pl-2">
            ...
            <Button
              ...
              size="icon-xs"
              onClick={onDelete}
              title="Delete Letter from import"
      ...
      <div className="p-5 space-y-4">
        {/* Title / Prompt Text */}
        <EditableField
          label="Letter Title / Prompt"
          value={item.title || ""}
          multiline
          placeholder="Enter letter prompt text..."
...
export function ImportLetterView() {
  const router = useRouter()
  const importMutation = useImportLetters()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Letter items."}`)
    }
  }
  ...
  const handleDeleteParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.info(`Removed letter #${idx + 1} from import list.`)
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.success(`Duplicated letter #${idx + 1}.`)
  }

  const handleAddNewQuestionCard = () => {
    const newLetter = {
      title: "New Letter Prompt...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
    const next = [...parsedItems, newLetter]
    syncParsedItemsToText(next)
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ letters: payload })
      toast.success(`Successfully imported ${res.importedCount} Letters!`)
      setTimeout(() => {
        router.push("/letters")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import letters.")
    }
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload letters or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Letter items will be assigned subject from the selection below.
          </p>
        </CardHeader>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-800 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {parsedItems.length} letter(s) parsed successfully.
                  </p>
                </div>
              </div>
              <Button
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-headline-md text-base font-bold text-on-surface flex items-center gap-2">
                    ...
                    Card Preview ({parsedItems.length} Letters)
                  </h4>
                </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            ...
            >
              ...
              <span>
                {importMutation.isPending
                  ? "Importing..."
                  : `Import ${parsedItems.length} Letters`}
              </span>
            </Button>
          </div>
        </CardContent>
