...
interface EditableGenderChangeCardProps {
  index: number
  item: any
...
function EditableGenderChangeCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableGenderChangeCardProps) {
    ...
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Word Text */}
          <EditableField
            label="Word (শব্দ / পুরুষবাচক শব্দ)"
            value={item.word || item.question || item.name || item.title || ""}
            placeholder="Enter word text (e.g. ছাত্র, নর)..."
            onSave={(newVal) => onChange({ ...item, word: newVal })}
          />

          {/* Gender Word */}
          <EditableField
            label="Gender Word (স্ত্রীবাচক শব্দ / রূপান্তর)"
            value={item.genderWord || ""}
            placeholder="Enter gender word (e.g. ছাত্রী, নারী)..."
            onSave={(newVal) => onChange({ ...item, genderWord: newVal })}
          />
        </div>
...
export function ImportGenderChangeView() {
  const router = useRouter()
  const importMutation = useImportGenderChange()
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        validated.push({
          ...
          genderWord: item.genderWord || item.meaning ? String(item.genderWord || item.meaning).trim() : null,
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Gender Change items."}`)
    }
  }
  ...
  const handleImport = async () => {
    ...
    try {
      const payload = parsedItems.map((item) => ({
        ...
        word: item.word.trim(),
        genderWord: item.genderWord ? item.genderWord.trim() : null,
      ...
      toast.success(`Successfully imported ${res.importedCount} Gender Change Entries!`)
      setTimeout(() => {
        router.push("/gender-change")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import gender change words.")
    }
  }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Gender Change Words from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload chapter-based words & gender counterparts (লিঙ্গ পরিবর্তন) or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Chapter is required. Gender change items will be assigned subject and chapter from the selection below.
          </p>
        </CardHeader>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-6">
        ...
        >
          <Link href="/gender-change">Cancel</Link>
        </Button>
        ...
        >
          {importMutation.isPending ? (
            ...
            <>
              ...
              <span>Import {parsedItems.length} Gender Change Entries</span>
            </>
          )}
        </Button>
      </div>
