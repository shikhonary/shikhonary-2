...
const sampleJsonTemplate = `[
  {
    "content": "Fill in the blanks with the correct form of verbs or change sentences given in the passage:\\nAn ideal student always (learn) his lessons properly. He never (waste) his valuable time in idleness. He knows that time once gone is gone forever. So, he utilizes every moment in useful work. Thus he succeeds in life.",
    "reference": ["Dhaka Board 2024"],
    "difficulty": "MEDIUM",
    "popularityCount": 0
  },
  {
    "options": [
      ...
      "(b) A healthy man is the happiest person. (Negative)",
      "(c) An unhealthy man cannot enjoy life. (Affirmative)",
      "(d) He should take care of his health. (Passive)",
      "(e) Everybody knows it. (Interrogative)"
    ],
    "reference": ["Rajshahi Board 2023"],
    "difficulty": "MEDIUM",
    "popularityCount": 0
  }
]`
...
function RenderDirectiveHighlights({ text }: { text: string }) {
  if (!text) return null
      ...
      {isEditing ? (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            ...
            >
              {renderFormatted ? (
                <RenderDirectiveHighlights text={value || placeholder} />
              ) : (
                value || placeholder
              )}
            </span>
...
interface EditableChangingSentenceCardProps {
  index: number
  item: any
...
function EditableChangingSentenceCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableChangingSentenceCardProps) {
  ...
  const handleSaveOptions = (val: string) => {
    const opts = val
      .split("\n")
      .map((o) => o.trim())
      .filter(Boolean)
  ...
  const optionsString = Array.isArray(item.options) ? item.options.join("\n") : ""
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        {/* Main Prompt / Direction / Passage */}
        <EditableField
          label="Passage / Prompt Direction (Optional if options provided)"
          value={item.content || ""}
          multiline
          renderFormatted
          placeholder="e.g. Passage text or prompt direction (Leave blank if only using discrete sentences below)..."
          onSave={(newVal) => onChange({ ...item, content: newVal || null })}
        />

        {/* Options / Discrete Sentences */}
        <div className="border-t border-outline-variant/30 pt-3">
          <div className="flex items-center justify-between mb-1">
            <span className="font-label-sm text-xs font-bold uppercase tracking-wider text-outline">
              Sentences with Directives (One per line)
            </span>
            <span className="text-[11px] font-semibold text-outline">
              {item.options?.length || 0} sentence(s)
            </span>
          </div>
          <EditableField
            value={optionsString}
            multiline
            renderFormatted
            placeholder="e.g. (a) Health is wealth. (Complex)&#10;(b) A healthy man is the happiest person. (Negative)"
...
export function ImportChangingSentenceView() {
  const router = useRouter()
  const importMutation = useImportChangingSentences()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        const rawContent = item.content !== undefined && item.content !== null ? String(item.content).trim() : ""
        const contentText = rawContent || (item.name ? String(item.name).trim() : "")
        ...
        if (!contentText && optionsArray.length === 0) {
          errors.push(`Item #${itemNum}: Must provide either 'content' (passage/prompt) or at least one sentence in 'options'.`)
        }
        if (!subjectId) {
      ...
      setParseError(
        `Validation Error: ${err.message || "Could not validate Changing Sentences items."}`
      )
    }
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    next.splice(idx + 1, 0, {
      ...itemToCopy,
      content: itemToCopy.content ? `${itemToCopy.content} (Copy)` : null,
    })
    syncParsedItemsToText(next)
  ...
  const handleImport = async () => {
    ...
    try {
      const payload = parsedItems.map((item) => ({
        ...
        content: item.content && typeof item.content === "string" && item.content.trim() ? item.content.trim() : null,
      ...
      const res = await importMutation.mutateAsync({ changingSentences: payload })
      toast.success(`Successfully imported ${res.count ?? payload.length} Changing Sentences questions!`)
      setTimeout(() => {
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Changing Sentences from JSON
          </h2>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Question items will be assigned the subject from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-800 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {parsedItems.length} question(s) parsed successfully.
                  </p>
                </div>
              </div>
              <Button
          ...
          {parsedItems.length > 0 && (
            <div className="border-t border-outline-variant/30 pt-6 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-headline-md text-base font-bold text-on-surface flex items-center gap-2">
                    ...
                    Card Preview ({parsedItems.length} Questions)
                  </h4>
                </div>
              ...
              {/* Grid of Changing Sentences Preview Cards */}
              <div className="space-y-6">
                {parsedItems.map((item, idx) => (
                  <EditableChangingSentenceCard
                    key={idx}
                    index={idx}
                    item={item}
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/changing-sentences")}
            ...
            >
              ...
              <span>
                {importMutation.isPending
                  ? "Importing..."
                  : `Import ${parsedItems.length} Questions`}
              </span>
            </Button>
          </div>
        </CardContent>
