...
export function CreateWordMeaningView() {
  const router = useRouter()
  const createMutation = useCreateWordMeaning()
  ...
  } = useForm<CreateWordMeaningFormData>({
    resolver: zodResolver(createWordMeaningFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: CreateWordMeaningFormData) => {
    ...
    try {
      ...
      const msg = err.message || "Failed to create word meaning entry"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mb-2 font-headline-md text-3xl font-extrabold text-primary">
            Create Word Meaning Entry
          </h2>
          <p className="font-body-md text-on-surface-variant leading-relaxed text-sm">
            Add a new word and its meaning (শব্দার্থ) to the chapter-based question bank.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card className="border border-outline-variant/30 bg-surface-container-lowest p-6 shadow-xs rounded-xl">
          ...
          <CardContent className="p-0 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                {...register("word")}
                placeholder="Enter word (e.g. পরিবেশ, বিদ্যালয়, সমুদ্র)..."
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {isSubmitting ? "Creating..." : "Save Word Meaning"}
          </Button>
        </div>
      </form>
    </div>
