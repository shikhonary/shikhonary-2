...
export function RoleListHeader() {
  const openModal = useCreateRoleModalStore((state) => state.openModal)
    ...
    <section className="mb-6 sm:mb-8 flex flex-col justify-between gap-4 sm:gap-6 md:flex-row md:items-end">
      <div>
        ...
        <p className="max-w-2xl font-body-md text-xs sm:text-sm md:text-base leading-relaxed text-on-surface-variant">
          Manage system roles and access privileges across the entire portal ecosystem.
        </p>
      </div>
      <Button
        onClick={openModal}
      ...
      >
        ...
        <span className="relative z-10">Add New Role</span>
      </Button>
    </section>
  )
}
