...
const createPartsOfSpeechFormSchema = z.object({
...
type CreatePartsOfSpeechFormData = z.infer<typeof createPartsOfSpeechFormSchema>

/**
 * Preview renderer for underlined text
 */
...
export function CreatePartsOfSpeechView() {
  const router = useRouter()
  const createMutation = useCreatePartsOfSpeech()
  ...
  } = useForm<CreatePartsOfSpeechFormData>({
    resolver: zodResolver(createPartsOfSpeechFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: CreatePartsOfSpeechFormData) => {
    ...
    try {
      ...
      toast.success("Parts of Speech question created successfully.")
      setTimeout(() => {
        router.push("/parts-of-speech")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to create Parts of Speech question"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            New Parts of Speech Question
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Add a new parts of speech identification question with underlined words, academic properties, and references.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="border-b border-outline-variant/40 bg-surface-container-lowest p-4 sm:p-8 flex flex-row items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="font-body-sm text-xs sm:text-sm text-on-surface-variant">
              Fill in academic details and passage content with underlined words.
            </p>
          </div>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="content-textarea"
                rows={5}
                placeholder="e.g. Read the text and identify the parts of speech of the underlined words: Honesty is the <u>best</u> policy. A <u>poor</u> woodcutter lived in a <u>small</u> village..."
