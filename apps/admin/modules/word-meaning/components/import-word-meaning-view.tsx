...
interface EditableWordMeaningCardProps {
  index: number
  item: any
...
function EditableWordMeaningCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableWordMeaningCardProps) {
      ...
      <div className="p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <EditableField
            ...
            placeholder="Enter word text (e.g. ক্ষ, পরিবেশ)..."
          ...
          <EditableField
            label="Meaning (অর্থ)"
            value={item.meaning || ""}
            placeholder="Enter meaning/definition (e.g. খিয়)..."
            multiline={true}
            isMath={false}
            onSave={(newVal) => onChange({ ...item, meaning: newVal })}
          />
        </div>
...
export function ImportWordMeaningView() {
  const router = useRouter()
  const importMutation = useImportWordMeaning()
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        validated.push({
          ...
          meaning: item.meaning ? String(item.meaning).trim() : null,
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Word Meaning items."}`)
    }
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success(`Successfully imported ${res.importedCount} Word Meaning Entries!`)
      setTimeout(() => {
        router.push("/word-meaning")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import word meanings.")
    }
  }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Word Meanings from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload chapter-based words & meanings (শব্দার্থ) or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Chapter is required. Word meaning items will be assigned subject and chapter from the selection below.
          </p>
        </CardHeader>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-6">
        ...
        >
          <Link href="/word-meaning">Cancel</Link>
        </Button>
        ...
        >
          {importMutation.isPending ? (
            ...
            <>
              ...
              <span>Import {parsedItems.length} Word Meaning Entries</span>
            </>
          )}
        </Button>
      </div>
