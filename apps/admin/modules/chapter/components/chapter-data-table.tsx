...
export function ChapterDataTable({
  items,
  isLoading,
  isError,
  onEdit,
    ...
    <div className="overflow-hidden rounded-2xl border border-outline-variant bg-surface-container-lowest shadow-xs">
      {isLoading ? (
        <div className="p-8 text-center text-on-surface-variant text-sm font-body-md flex items-center justify-center gap-3">
          ...
          <span>Loading academic chapters...</span>
        </div>
      ) : isError ? (
        <div className="p-8 text-center text-error">
          <p className="font-body-md font-medium">Failed to load academic chapters.</p>
        </div>
      ) : items.length === 0 ? (
        <div className="p-12 text-center flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-bold text-on-surface">No academic chapters found.</p>
          <Link href="/chapters/create">
            <Button size="sm" className="mt-2 rounded-lg bg-primary-container text-on-primary-container hover:bg-primary hover:text-white transition-colors font-bold">
              <Plus className="h-4 w-4 mr-1" /> Add First Chapter
            </Button>
          </Link>
        </div>
      ) : (
            ...
            {items.map((ch) => {
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-on-surface-variant truncate">{ch.nameBn}</p>
                        <div className="mt-1.5 flex flex-wrap gap-1 items-center">
                          <Badge variant="outline" className="rounded-md border-outline-variant px-1.5 py-0 text-[9px] font-mono font-bold text-outline">
                            Pos: {ch.position}
                          </Badge>
                          <Badge
                            variant="outline"
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-white border border-outline-variant shadow-md rounded-xl p-1.5 min-w-[140px]">
                        ...
                        <DropdownMenuItem
                          onClick={() => onToggleActive(ch.id, ch.isActive)}
                          className="cursor-pointer gap-2.5 rounded-lg px-3 py-2 text-xs font-medium text-on-surface hover:bg-surface-container-high"
                        >
                          {ch.isActive ? (
                            <>
                        ...
                        <DropdownMenuItem
                          onClick={() => onDelete(ch.id, ch.nameEn)}
                  ...
                  <div className="flex flex-col gap-1 border-t border-outline-variant/20 pt-2 text-[10px] text-on-surface-variant font-medium">
                    {ch.subject && (
                      <div className="flex items-center gap-1.5">
                        <BookOpen className="h-3.5 w-3.5 text-primary shrink-0" />
                        <span>Subject:</span>
                        <span className="font-bold text-on-surface truncate">{ch.subject.nameEn}</span>
                      </div>
                    )}
                    {ch.academicYear && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>Year:</span>
                        <span className="font-bold text-on-surface truncate">{ch.academicYear.nameEn}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <Table className="w-full text-left font-body-md">
              <TableHeader className="bg-surface-container-low border-b border-outline-variant">
                <TableRow className="border-b border-outline-variant bg-surface-container-low hover:bg-surface-container-low">
                  <TableHead className="px-6 py-4 font-label-sm font-semibold tracking-wider text-outline uppercase h-auto min-w-[80px]">
                    Pos
                  </TableHead>
              ...
              <TableBody className="divide-y divide-outline-variant/30">
                {items.map((ch) => {
                  return (
                    <TableRow key={ch.id} className="hover:bg-surface-container-low transition-all duration-200 ease-in-out group border-b border-outline-variant/30">
                      <TableCell className="py-4 group-hover:py-5 px-6 text-on-surface font-mono font-bold transition-all duration-200 ease-in-out">
                        {ch.position}
                      </TableCell>
                      ...
                      <TableCell className="py-4 group-hover:py-5 px-6 text-on-surface-variant transition-all duration-200 ease-in-out">
                        {ch.academicYear ? (
                          <div className="flex items-center gap-1.5 text-xs font-semibold text-on-surface">
                            <Calendar className="h-3.5 w-3.5 text-primary shrink-0" />
                            <span>{ch.academicYear.nameEn}</span>
                          </div>
                        ) : (
                          <span className="text-outline/50">-</span>
                        )}
