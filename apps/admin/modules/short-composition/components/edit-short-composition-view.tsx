...
import {
  useShortCompositionById,
  useUpdateShortComposition,
...
const editShortCompositionFormSchema = z.object({
  classId: z.string().min(1, "Please select an academic class"),
  subjectId: z.string().min(1, "Please select a subject"),
  title: z.string().min(1, "Short Composition topic / title prompt is required"),
  wordLimit: z
    .string()
    .optional()
    .refine((val) => !val || (!isNaN(Number(val)) && Number(val) > 0), {
      message: "Word limit must be a positive integer",
    }),
...
type EditShortCompositionFormData = z.infer<typeof editShortCompositionFormSchema>

export function EditShortCompositionView({ id: propId }: { id?: string } = {}) {
  const router = useRouter()
  const params = useParams()
  const shortCompositionId = propId || (typeof params?.id === "string" ? params.id : "")

  const { data: itemData, isLoading: isItemLoading, isError } = useShortCompositionById(shortCompositionId)
  const updateMutation = useUpdateShortComposition()
  ...
  } = useForm<EditShortCompositionFormData>({
    resolver: zodResolver(editShortCompositionFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  useEffect(() => {
    if (itemData) {
      ...
      reset({
        classId: item.subject?.classSubjects?.[0]?.classId || "",
        subjectId: item.subjectId || "",
        title: item.title || "",
        wordLimit: item.wordLimit ? String(item.wordLimit) : "",
        referenceText: Array.isArray(item.reference) ? item.reference.join(", ") : "",
        difficulty: item.difficulty || QUESTION_DIFFICULTY.MEDIUM,
  ...
  const onSubmit = async (data: EditShortCompositionFormData) => {
    ...
    try {
      ...
      const parsedWordLimit = data.wordLimit && data.wordLimit.trim() !== ""
        ? parseInt(data.wordLimit, 10)
        : null
      ...
      toast.success("Short Composition updated successfully.")
      setTimeout(() => {
        router.push("/short-composition")
      }, 500)
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to update Short Composition")
      toast.error(err.message || "Failed to update Short Composition")
    }
  }
      ...
      <div className="mx-auto max-w-4xl py-12 text-center">
        <div className="rounded-2xl border border-dashed border-error/40 bg-error/5 p-12">
          <h2 className="font-headline-md text-xl font-bold text-error">
            Short Composition Not Found
          </h2>
          <p className="mt-1 font-body-md text-sm text-outline">
            The short composition question you are trying to edit does not exist or has been removed.
          </p>
          <div className="mt-6">
            <Button asChild className="rounded-xl font-bold bg-primary text-white">
              <Link href="/short-composition">Back to Short Compositions</Link>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          >
            ...
            <span>Back to Short Compositions</span>
          </Link>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-headline-md text-xl sm:text-2xl font-extrabold text-on-surface">
                Edit Short Composition
              </h1>
              <p className="font-body-md text-xs text-on-surface-variant">
                Modify topic, word limit, examination references, and metadata.
              </p>
            </div>
          </div>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-sm">
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Composition Topic / Title Statement */}
            <div className="space-y-2">
              <Label htmlFor="title" className="text-sm font-bold text-on-surface">
                Composition Topic / Prompt <span className="text-red-500">*</span>
              </Label>
              <Textarea
                id="title"
                rows={5}
                placeholder={`e.g. Write a short composition on "Discipline" or "A Journey by Boat".`}
                {...register("title")}
            ...
            {/* Word Limit, Reference Tags & Metadata */}
            <div className="space-y-4">
              <h3 className="font-headline-sm text-sm font-bold uppercase tracking-wider text-outline">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {/* Word Limit (Optional) */}
                <div className="space-y-2">
                  <Label htmlFor="wordLimit" className="text-sm font-bold text-on-surface">
                    Word Limit (Optional)
                  </Label>
                  <Input
                    id="wordLimit"
                    type="number"
                    min="1"
                    placeholder="e.g. 200 (in 200 words)"
                    {...register("wordLimit")}
                  ...
                  />
                  {errors.wordLimit && (
                    <p className="text-xs font-semibold text-red-500">{errors.wordLimit.message}</p>
                  )}
                  <p className="text-xs text-outline">
                    Leave blank if no specific word limit.
                  </p>
                </div>
