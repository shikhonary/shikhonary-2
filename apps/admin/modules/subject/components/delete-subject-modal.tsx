...
import { useDeleteSubject } from "../services/use-subject"
...
export function DeleteSubjectModal() {
  const { isOpen, subjectId, subjectName, closeModal } =
    useDeleteSubjectModalStore()
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteMutation.mutateAsync({ id: subjectId })
      toast.success(
        `Academic subject "${subjectName || "Subject"}" deleted successfully.`
      )
      closeModal()
    } catch (err: any) {
      ...
      >
        ...
        <DialogHeader className="pt-1">
          <DialogDescription className="font-body-md text-sm leading-relaxed text-on-surface-variant">
            ...
            ? All chapters and class mapping records linked to this subject template will be permanently removed.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-700">
          ...
          <p className="leading-snug">
            This process is permanent and cannot be undone. Please verify no active class curriculum maps rely on this subject.
          </p>
        </div>
