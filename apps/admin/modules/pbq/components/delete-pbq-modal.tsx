...
import { useDeletePbq, useBulkDeletePbqs } from "../services/use-pbq"
...
interface DeletePbqModalProps {
  onSuccess?: () => void
}

export function DeletePbqModal({ onSuccess }: DeletePbqModalProps) {
  const { isOpen, pbqId, pbqContext, selectedIds, closeModal } =
    useDeletePbqModalStore()

  const deleteMutation = useDeletePbq()
  const bulkDeleteMutation = useBulkDeletePbqs()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.deletedCount} PBQs.`)
      } else if (pbqId) {
        await deleteMutation.mutateAsync({ id: pbqId })
        toast.success("PBQ question deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete PBQ")
    }
  }
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} PBQs?` : "Delete Passage-Based Question?"}
            </DialogTitle>
            ...
            {isBulk ? (
              <>
                ...
                <span className="font-bold text-on-surface">
                  {selectedIds.length} selected passage-based questions
                </span>
                ? All associated sub-questions, reference records, and analytics will be permanently removed.
              </>
            ) : (
              <>
                Are you sure you want to delete the passage-based question:
                <strong className="block mt-2 rounded-lg border border-outline-variant/40 bg-surface-container-lowest p-3 text-xs font-mono text-on-surface-variant line-clamp-2 break-all font-medium">
                  &quot;{pbqContext || "(No Passage Context - Question ID: " + pbqId + ")"}&quot;
                </strong>
              </>
            )}
          </DialogDescription>
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-700">
          ...
          <p className="leading-snug">
            This action cannot be undone. Associated test entries will be affected.
          </p>
        </div>
