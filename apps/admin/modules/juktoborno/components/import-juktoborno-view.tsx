...
function EditableJuktobornoCard({
  index,
  item,
  onChange,
  onDelete,
...
export function ImportJuktobornoView() {
    ...
    try {
      ...
      data.forEach((item: any, idx: number) => {
        ...
        if (Array.isArray(item.reference)) {
          refArr = item.reference.map((r: any) => String(r).trim()).filter(Boolean)
        } else if (typeof item.reference === "string") {
          refArr = item.reference.split(",").map((r: string) => r.trim()).filter(Boolean)
        }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    next.splice(idx + 1, 0, {
      ...itemToCopy,
      juktoborno: `${itemToCopy.juktoborno} (Copy)`,
    })
    syncParsedItemsToText(next)
  ...
  const handleImport = async () => {
    ...
    try {
      const payload = parsedItems.map((item) => ({
        ...
        juktoborno: item.juktoborno.trim(),
      ...
      toast.success(`Successfully imported ${res.importedCount} Juktoborno Items!`)
      setTimeout(() => {
        router.push("/juktoborno")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import juktoborno items.")
    }
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Juktoborno from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload chapter-based conjunct letters (যুক্তবর্ণ) or edit individual cards before importing into the question bank.
          </p>
        </div>
        ...
        <div className="space-y-6 animate-fade-in mt-8">
          <div className="flex items-center justify-between">
            <h3 className="font-headline-md text-lg font-bold text-primary flex items-center gap-2">
              ...
              Parsed Juktoborno ({parsedItems.length} items ready for import)
            </h3>
        ...
        >
          {importMutation.isPending ? (
            ...
            <>
              ...
              <span>Import {parsedItems.length} Juktoborno Items</span>
            </>
          )}
        </Button>
      </div>
