...
const ShortCompositionEditableText = ({ 
  text, 
  itemKey, 
  defaultStyle,
  className = ""
...
export const formatShortCompositionPrompt = (
  rawLabel: string | undefined | null,
  title: string | undefined | null,
  wordLimit: number | string | null | undefined
): string => {
  const cleanTitle = (title || "").trim();
  if (!cleanTitle) return "";

  // If the title already looks like a full prompt (e.g. starts with "Write a..."), return it with word limit if needed
  if (/^write\s+(a\s+)?(short\s+)?composition/i.test(cleanTitle)) {
    if (wordLimit && !cleanTitle.toLowerCase().includes("word")) {
      return `${cleanTitle.replace(/[.]+$/, "")} in ${wordLimit} words.`;
    }
    return cleanTitle;
  }

  const prefix = (rawLabel && rawLabel.trim())
    ? rawLabel.trim().replace(/[:.]+$/, "")
    : "Write a short composition on";

  const quotedTitle = cleanTitle.startsWith('"') || cleanTitle.startsWith("'")
    ? cleanTitle
    : `"${cleanTitle}"`;

  const wordLimitSuffix = wordLimit ? ` in ${wordLimit} words.` : ".";

  return `${prefix} ${quotedTitle}${wordLimitSuffix}`;
};

export const ShortCompositionBlock = ({ item }: { item: any }) => {
  ...
  const handleRemove = () => {
    ...
    removeQuestion({
      ...
      questionType: "SHORT_COMPOSITION",
    });
  };
  ...
  const renderSubQuestionLabel = (labelStr: string) => {
    if (!labelStr || item.totalQuestions <= 1) return null;
  ...
  return (
    <div className={`group relative -mx-4 px-4 hover:bg-muted/10 rounded-lg transition-colors flex flex-col break-inside-avoid ${item.isFirstShortComposition ? "pt-0.5 pb-0" : "py-0 my-0"}`}>
      {/* Hover Controls */}
      ...
      <AddAlternativeModal
        ...
        primaryQuestionType="SHORT_COMPOSITION"
