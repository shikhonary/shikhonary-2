...
import { useDeleteSynonym, useBulkDeleteSynonym } from "../services/use-synonym"

export function DeleteSynonymModal() {
  const { isOpen, synonymId, wordText, selectedIds, closeModal } = useDeleteSynonymModalStore()
  const deleteMutation = useDeleteSynonym()
  const bulkDeleteMutation = useBulkDeleteSynonym()
  ...
  const handleDelete = async () => {
    if (isBulk) {
      ...
      deleteMutation.mutate(
        ...
        {
          onSuccess: () => {
            toast.success("Successfully deleted synonym entry")
            closeModal()
          },
          onError: (err) => {
            toast.error(err.message || "Failed to delete synonym entry")
          },
        }
      )
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-bold text-on-surface">
                {isBulk ? `Delete ${selectedIds.length} Words` : "Delete Synonym Entry"}
              </DialogTitle>
