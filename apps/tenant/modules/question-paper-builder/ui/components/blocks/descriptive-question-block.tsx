...
const DescriptiveQuestionEditableText = ({ 
  text, 
  itemKey, 
  defaultStyle,
  className = ""
...
export const DescriptiveQuestionBlock = ({ item }: { item: any }) => {
  ...
  const marksPerQuestion = defaultMark;
  ...
  const handleRemove = () => {
    ...
    removeQuestion({
      ...
      questionType: "DESCRIPTIVE_QUESTION",
    });
  };
  ...
  const questionText = data.question || data.title || data.name || "";
  const answerText = data.answer || "";

  return (
    <div className={`group relative -mx-4 px-4 hover:bg-muted/10 rounded-lg transition-colors flex flex-col break-inside-avoid ${item.isFirstDescriptiveQuestion ? "pt-0.5 pb-0" : "py-0 my-0"}`}>
      {/* Hover Controls */}
      ...
      {item.isFirstDescriptiveQuestion && (
        <div className="flex justify-between items-start w-full mb-1">
          ...
          >
            ...
            <EditableSectionLabel
              ...
              fallbackLabel="নিচের প্রশ্নগুলোর উত্তর দাও:"
              questionType="DESCRIPTIVE_QUESTION"
        ...
        <div className="flex gap-1 flex-1 relative flex-col">
          <div className="flex gap-2 items-start w-full">
            ...
            <div className="flex-1 w-full min-w-0">
              <DescriptiveQuestionEditableText 
                text={questionText}
              ...
              />
              {answerText && (
                <div className="mt-1 text-xs text-muted-foreground/80 italic font-mono border-l-2 border-primary/20 pl-2">
                  <RenderMath text={`উত্তর: ${answerText}`} />
                </div>
              )}
            </div>
          </div>
      ...
      <AddAlternativeModal
        ...
        primaryQuestionType="DESCRIPTIVE_QUESTION"
