...
export interface FillInTheBlanksWithCluesItem {
  id: string
  content: string
  clues: string[]
  reference: string[]
...
interface FillInTheBlanksWithCluesTableProps {
  items: FillInTheBlanksWithCluesItem[]
  isLoading: boolean
  isError: boolean
...
/**
 * Renders clues in a clean 5-column table grid layout with black borders
 */
function CluesTableGrid({ clues }: { clues: string[] }) {
  ...
  return (
    <div className="w-full max-w-2xl overflow-x-auto my-2">
      <table className="w-full border-collapse border border-black text-center text-xs sm:text-sm bg-white">
        <tbody>
          {rows.map((row, rIdx) => (
            <tr key={rIdx}>
              {Array.from({ length: columnsCount }).map((_, cIdx) => {
                ...
                return (
                  <td
                    key={cIdx}
                    className={cn(
                      "border border-black px-3 py-2 font-medium text-black",
...
export function FillInTheBlanksWithCluesTable({
  items,
  isLoading,
  isError,
  onDelete,
  ...
  onPageChange,
  onLimitChange,
}: FillInTheBlanksWithCluesTableProps) {
        ...
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="font-body-md text-sm text-outline max-w-md">
              Try adjusting filters or add a new fill in the blanks with clues question to the bank.
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                  ...
                  <div className="flex items-start gap-3 md:gap-4 flex-1 min-w-0 w-full relative md:static">
                    ...
                    <div className="flex-1 space-y-4 min-w-0">
                      ...
                      {/* CLUES BOX PREVIEW (Table/Grid Style with 5 items per row) */}
                      {Array.isArray(item.clues) && item.clues.length > 0 && (
                        <CluesTableGrid clues={item.clues} />
                      )}
                  ...
                  <div className="flex md:flex-col justify-end items-center gap-2 shrink-0 border-t md:border-t-0 border-outline-variant/40 pt-3 md:pt-0 w-full md:w-auto">
                    <Link
                      href={`/fill-in-the-blanks-with-clues/${item.id}/edit`}
