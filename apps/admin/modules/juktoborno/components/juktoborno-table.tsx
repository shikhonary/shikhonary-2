...
interface JuktobornoTableProps {
  items: JuktobornoItem[]
  isLoading: boolean
  isError: boolean
  onDelete: (id: string, juktobornoSnippet: string) => void
...
export function JuktobornoTable({
  items,
  isLoading,
  isError,
  onDelete,
      ...
      {isLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading Juktoborno Items...</span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-error rounded-xl border border-error/30 bg-error-container/20">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">
              Error loading juktoborno items. Please try refreshing.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="font-headline-md text-xl font-bold text-on-surface">
              No Juktoborno Items Found
            </p>
            <p className="font-body-md text-sm text-outline max-w-md">
              Try adjusting filters or add a new juktoborno entry to the bank.
            </p>
          </div>
        </div>
      )}
                    ...
                    <div className="flex-1 space-y-4 min-w-0">
                      ...
                      <div className="rounded-xl border border-secondary/20 bg-secondary-container/10 p-4 text-sm text-on-surface leading-relaxed space-y-2">
                        <div className={cn(
                          ...
                          /[\u0980-\u09FF]/.test(item.juktoborno) && "font-solaiman text-xl"
                        )}>
                          <RenderMath text={item.juktoborno} isMath={true} />
                        </div>
                      </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border border-outline-variant bg-surface-container-low rounded-xl p-4">
          <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-start">
            <p className="font-body-md text-xs sm:text-sm text-on-surface-variant">
              Showing <span className="font-bold">{displayStart}-{displayEnd}</span> of <span className="font-bold">{totalItems}</span> juktoborno items
            </p>
            {onLimitChange && (
              <div className="flex items-center gap-2">
