...
import { useCreateSummary, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-summary"
...
const createSummaryFormSchema = z.object({
...
type CreateSummaryFormData = z.infer<typeof createSummaryFormSchema>
...
export function CreateSummaryView() {
  ...
  } = useForm<CreateSummaryFormData>({
    resolver: zodResolver(createSummaryFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  const onSubmit = async (data: CreateSummaryFormData) => {
    ...
    try {
      ...
      setErrorMessage(err.message || "Failed to create Summary")
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="font-headline-md text-xl font-bold text-on-surface">
                Create New Summary / Précis
              </CardTitle>
              <p className="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">
                Add a new passage or poem for summary / précis writing (সারাংশ / সারমর্ম লিখন) with reference citations.
              </p>
            </div>
          </div>
        </CardHeader>
