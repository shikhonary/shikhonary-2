...
interface TaxPaymentFilterProps {
  fiscalYears: any[]
  wards: any[]
  stats: any
...
export function TaxPaymentFilter({
  fiscalYears,
  wards,
  stats,
  setCurrentPage,
}: TaxPaymentFilterProps) {
        ...
        <div className="flex flex-col gap-2.5 rounded-xl border border-border/40 bg-card/40 p-3 text-xs sm:flex-row sm:items-center sm:justify-between sm:bg-transparent sm:border-0 sm:p-0 sm:px-1">
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            {statusFilter !== "all" && (
              ...
              >
                <span>অবস্থা: {statusFilter === "paid" ? "পরিশোধিত" : "অনাদায়ী"}</span>
                <button
                  type="button"
            ...
            {fiscalYearFilter !== "all" && (
              ...
              >
                <span>
                  অর্থবছর:{" "}
                  {fiscalYears.find((fy: any) => fy.id === fiscalYearFilter)?.year || "নির্বাচিত অর্থবছর"}
                </span>
                <button
                  type="button"
