...
const sampleJsonTemplate = `[
  {
    "content": "Read the following text and identify the parts of speech of the underlined words:\\nHonesty is the <u>best</u> policy. A <u>poor</u> woodcutter lived in a <u>small</u> village. He worked <u>diligently</u> every day and lived <u>happily</u>.",
...
function RenderContentWithUnderline({ text }: { text: string }) {
  if (!text) return null
      ...
      {parts.map((part, index) => {
        if (part.toLowerCase().startsWith("<u>") && part.toLowerCase().endsWith("</u>")) {
          ...
          return (
            <span
              key={index}
              className="inline-block px-1 py-0.5 mx-0.5 rounded font-bold underline decoration-amber-500 decoration-2 bg-amber-500/10 text-amber-900"
            >
              {word}
            </span>
          )
      ...
      {isEditing ? (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            ...
            >
              {renderFormatted ? (
                <RenderContentWithUnderline text={value || placeholder} />
              ) : (
                value || placeholder
              )}
            </span>
...
interface EditablePartsOfSpeechCardProps {
  index: number
  item: any
...
function EditablePartsOfSpeechCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditablePartsOfSpeechCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        ...
        <EditableField
          label="Question / Passage Content with <u>Underlined Words</u>"
          value={item.content || item.name || ""}
          multiline
          renderFormatted
          placeholder="Enter question text with <u>underlined</u> words..."
...
export function ImportPartsOfSpeechView() {
  const router = useRouter()
  const importMutation = useImportPartsOfSpeech()
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        if (!contentText || typeof contentText !== "string" || !contentText.trim()) {
          errors.push(`Item #${itemNum}: Missing or invalid 'content' text.`)
        }
        if (!subjectId) {
      ...
      setParseError(
        `Validation Error: ${err.message || "Could not validate Parts of Speech items."}`
      )
    }
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ partsOfSpeech: payload })
      toast.success(`Successfully imported ${res.importedCount} Parts of Speech questions!`)
      setTimeout(() => {
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Parts of Speech from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload parts of speech questions or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Chapter is optional. Question items will be assigned subject and chapter from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          {parsedItems.length > 0 && (
            <div className="border-t border-outline-variant/30 pt-6 space-y-6">
              ...
              {/* Grid of Parts of Speech Preview Cards */}
              <div className="space-y-6">
                {parsedItems.map((item, idx) => (
                  <EditablePartsOfSpeechCard
                    key={idx}
                    index={idx}
                    item={item}
