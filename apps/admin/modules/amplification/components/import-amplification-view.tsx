...
import { useImportAmplifications } from "../services/use-amplification"
import { useAcademicClassesForSelection, useSubjectsForSelection, useChaptersForSelection } from "../../paragraph/services/use-paragraph"
...
import { repairJsonSyntax, findJsonErrorPosition } from "../../paragraph/components/import-paragraph-view"
...
const sampleJsonTemplate = `[
  {
    ...
    "difficulty": "MEDIUM",
    "popularityCount": 5,
    "reference": ["Dhaka Board 2024", "Comilla Board 2023"]
  },
  {
    "title": "লোভে পাপ, পাপে মৃত্যু",
    "difficulty": "EASY",
    "popularityCount": 12,
    "reference": ["Rajshahi Board 2022"]
  }
]`
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <span className={cn(
              ...
              value && /[\u0980-\u09FF]/.test(value) && "font-solaiman text-base"
            )}>
...
interface EditableAmplificationCardProps {
  index: number
  item: any
...
function EditableAmplificationCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableAmplificationCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2 border-l border-outline-variant/60 pl-2">
            ...
            <Button
              ...
              size="icon-xs"
              onClick={onDelete}
              title="Delete Amplification from import"
      ...
      <div className="p-5 space-y-4">
        {/* Title / Thought */}
        <EditableField
          label="Amplification Thought / Proverb Text"
          value={item.title || ""}
          multiline
          placeholder="Enter proverb thought..."
...
export function ImportAmplificationView() {
  const router = useRouter()
  const importMutation = useImportAmplifications()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      if (repaired && repaired !== text) {
        try {
          ...
          toast.success("Auto-repaired JSON formatting!")
        } catch {
          // Both failed
        }
      }
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        // Map either 'title' or fallback to 'name' to keep robust compatibility
        const titleText = item.title || item.name

        if (!titleText || typeof titleText !== "string" || !titleText.trim()) {
          errors.push(`Item #${itemNum}: Missing or invalid thought 'title' text.`)
        }
        if (!subjectId) {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate items."}`)
    }
  }
  ...
  const handleAddNewQuestionCard = () => {
    const newAmplification = {
      title: "New Quotation/Proverb thought...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
    const next = [...parsedItems, newAmplification]
    syncParsedItemsToText(next)
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ amplifications: payload })
      toast.success(`Successfully imported ${res.importedCount} Amplifications!`)
      setTimeout(() => {
        router.push("/amplifications")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import amplifications.")
    }
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Amplifications from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload amplifications or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Chapter is optional. Amplification items will be assigned subject and chapter from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-800 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {parsedItems.length} amplification(s) parsed successfully.
                  </p>
                </div>
              </div>
              <Button
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-headline-md text-base font-bold text-on-surface flex items-center gap-2">
                    ...
                    Card Preview ({parsedItems.length} Amplifications)
                  </h4>
                </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/amplifications")}
            ...
            >
              ...
              <span>
                {importMutation.isPending
                  ? "Importing..."
                  : `Import ${parsedItems.length} Amplifications`}
              </span>
            </Button>
          </div>
        </CardContent>
