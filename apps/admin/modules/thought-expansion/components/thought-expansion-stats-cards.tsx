...
interface ThoughtExpansionStatsCardsProps {
  totalCount?: number
  difficultyCounts?: {
    EASY: number
    MEDIUM: number
...
export function ThoughtExpansionStatsCards({
  totalCount = 0,
  difficultyCounts = { EASY: 0, MEDIUM: 0, HARD: 0 },
  isLoading = false,
}: ThoughtExpansionStatsCardsProps) {
  if (isLoading) {
    return (
      <div className="mb-6">
      ...
      <div className="hidden sm:grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 transition-all hover:shadow-sm">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-outline">
              Standard board maxims
            </p>
          </div>
        </div>

        {/* Hard Thought Expansions */}
        <div className="flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 transition-all hover:shadow-sm">
