...
import { useImportEssays, useAcademicClassesForSelection, useSubjectsForSelection } from "../services/use-essay"
...
interface EditableEssayCardProps {
  index: number
  item: any
...
function EditableEssayCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableEssayCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant/40 bg-surface-container-low/60 px-5 py-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-outline">
            Essay / Composition Topic Preview
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2 border-l border-outline-variant/60 pl-2">
            ...
            <Button
              ...
              size="icon-xs"
              onClick={onDelete}
              title="Delete Essay from import"
      ...
      <div className="p-5 space-y-4">
        {/* Title / Essay Topic Text */}
        <EditableField
          label="Essay Topic / Title Prompt"
          value={item.title || ""}
          multiline
          placeholder="Enter essay topic / prompt statement..."
...
export function ImportEssayView() {
  const router = useRouter()
  const importMutation = useImportEssays()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Essay items."}`)
    }
  }
  ...
  const handleDeleteParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.info(`Removed essay #${idx + 1} from import list.`)
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.success(`Duplicated essay #${idx + 1}.`)
  }
  ...
  const handleAddNewQuestionCard = () => {
    ...
    const next = [...parsedItems, newEssay]
    syncParsedItemsToText(next)
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to bulk import essays.")
    }
  }
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Essay items will be assigned subject from the selection below.
          </p>
        </CardHeader>
