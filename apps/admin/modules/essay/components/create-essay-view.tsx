...
import { useCreateEssay, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-essay"
...
type CreateEssayFormData = z.infer<typeof createEssayFormSchema>

export function CreateEssayView() {
  const router = useRouter()
  const createMutation = useCreateEssay()
  ...
  const {
    ...
    resolver: zodResolver(createEssayFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  const onSubmit = async (data: CreateEssayFormData) => {
    ...
    try {
      ...
      setErrorMessage(err.message || "Failed to create Essay")
      toast.error(err.message || "Failed to create Essay")
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-xs text-on-surface-variant">
                Add a new essay or composition topic prompt to the question bank.
              </p>
            </div>
          </div>
        </div>
