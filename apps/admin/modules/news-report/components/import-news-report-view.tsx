...
import { useImportNewsReports, useAcademicClassesForSelection, useSubjectsForSelection } from "../services/use-news-report"
...
const sampleJsonTemplate = `[
  {
    "title": "নিত্যপ্রয়োজনীয় দ্রব্যের অস্বাভাবিক মূল্যবৃদ্ধি জনজীবনকে দুর্বিষহ করে তুলেছে—এ বিষয়ে সংবাদপত্রে প্রকাশের উপযোগী একটি প্রতিবেদন প্রস্তুত করো।",
    "reference": ["Dhaka Board 2024", "Rajshahi Board 2023"],
    "difficulty": "MEDIUM",
    "popularityCount": 5
  },
  {
    "title": "তোমার কলেজে উদযাপিত বার্ষিক ক্রীড়া ও সাংস্কৃতিক প্রতিযোগিতার বিবরণ দিয়ে অধ্যক্ষ বরাবর একটি প্রাতিষ্ঠানিক প্রতিবেদন প্রণয়ন করো।",
...
interface EditableNewsReportCardProps {
  index: number
  item: any
...
function EditableNewsReportCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableNewsReportCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant/40 bg-surface-container-low/60 px-5 py-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-outline">
            News Report / Journalistic Prompt Preview
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2 border-l border-outline-variant/60 pl-2">
            ...
            <Button
              ...
              size="icon-xs"
              onClick={onDelete}
              title="Delete News Report from import"
      ...
      <div className="p-5 space-y-4">
        {/* Title / Report Prompt Text */}
        <EditableField
          label="Report Topic / Stimulus Prompt"
          value={item.title || ""}
          multiline
          placeholder="Enter report prompt / assignment topic..."
...
export function ImportNewsReportView() {
  const router = useRouter()
  const importMutation = useImportNewsReports()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate News Report items."}`)
    }
  }
  ...
  const handleDeleteParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.info(`Removed news report #${idx + 1} from import list.`)
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.success(`Duplicated news report #${idx + 1}.`)
  }

  const handleAddNewQuestionCard = () => {
    const newReport = {
      title: "নতুন প্রতিবেদন প্রম্পট বা বিবরণ...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
    const next = [...parsedItems, newReport]
    syncParsedItemsToText(next)
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ newsReports: payload })
      toast.success(`Successfully imported ${res.importedCount} News Reports!`)
      setTimeout(() => {
        router.push("/news-reports")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import news reports.")
    }
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import News Reports from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload news reports or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. News report items will be assigned subject from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-800 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {parsedItems.length} news report item(s) parsed successfully.
                  </p>
                </div>
              </div>
              <Button
          ...
          {parsedItems.length > 0 && (
            <div className="border-t border-outline-variant/30 pt-6 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-headline-md text-base font-bold text-on-surface flex items-center gap-2">
                    ...
                    Card Preview ({parsedItems.length} News Reports)
                  </h4>
                </div>
              ...
              {/* Grid of News Report Preview Cards */}
              <div className="space-y-6">
                {parsedItems.map((item, idx) => (
                  <EditableNewsReportCard
                    key={idx}
                    index={idx}
                    item={item}
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/news-reports")}
            ...
            >
              ...
              <span>
                {importMutation.isPending
                  ? "Importing..."
                  : `Import ${parsedItems.length} News Reports`}
              </span>
            </Button>
          </div>
        </CardContent>
