...
import { useTaxPayerDetailSheetStore } from "../store/use-tax-payer-detail-sheet-store"
...
export function TaxPayerDetailSheet({
  ...
  const store = useTaxPayerDetailSheetStore()
  ...
  const finalTaxPayerId = taxPayerId !== undefined ? taxPayerId : store.taxPayerId

  const { data: taxPayer, isLoading } = useQuery(
    trpc.taxPayer.byId.queryOptions(
      { id: finalTaxPayerId || "" },
      { enabled: !!finalTaxPayerId && finalOpen }
    )
  )
