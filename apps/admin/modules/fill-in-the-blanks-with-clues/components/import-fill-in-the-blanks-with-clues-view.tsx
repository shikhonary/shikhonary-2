...
import {
  useImportFillInTheBlanksWithClues,
...
const sampleJsonTemplate = `[
  {
    "clues": ["diligently", "happily", "honest", "wood", "forest", "help", "river", "reward"],
...
function EditableCard({
  ...
  const handleSaveClues = (val: string) => {
    const clues = val
      .split(",")
      .map((c) => c.trim())
      .filter(Boolean)
  ...
  const cluesString = Array.isArray(item.clues) ? item.clues.join(", ") : ""
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        {/* Clues Box */}
        <EditableField
          label="Words in the Box (Clues - Comma-separated)"
          value={cluesString}
          placeholder="e.g. diligent, honest, helper, river, wood..."
          onSave={handleSaveClues}
        />

        {/* Content / Passage */}
        <EditableField
          label="Passage Content with Blanks (a) _________"
          value={item.content || item.name || ""}
          multiline
          renderFormatted
          placeholder="Enter passage with (a) _________ gap markers..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-outline-variant/30 pt-3">
          <EditableField
            ...
            placeholder="e.g. Board 2024, Dhaka Board..."
...
export function ImportFillInTheBlanksWithCluesView() {
  const router = useRouter()
  const importMutation = useImportFillInTheBlanksWithClues()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        const contentText = item.content || item.name

        if (!contentText || typeof contentText !== "string" || !contentText.trim()) {
          errors.push(`Item #${itemNum}: Missing or invalid 'content' passage text.`)
        }
        if (!subjectId) {
        ...
        const clues = Array.isArray(item.clues)
          ? item.clues
          : typeof item.clues === "string" && item.clues.trim()
          ? item.clues.split(",").map((c: string) => c.trim()).filter(Boolean)
          : []
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    next.splice(idx + 1, 0, {
      ...itemToCopy,
      content: `${itemToCopy.content} (Copy)`,
    })
    syncParsedItemsToText(next)
  ...
  const handleAddNewQuestionCard = () => {
    const newItem = {
      clues: ["word1", "word2", "word3"],
      content: "Complete the text with words from the box:\nPassage with (a) _________ and (b) _________...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload cloze test questions or edit cards with clue boxes and passage blanks before importing.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Question items will inherit this selection.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-white text-neutral-900 border border-outline-variant">
                  <SelectItem value="" className="text-neutral-900">None / Optional</SelectItem>
                  {chapters.map((ch) => (
          ...
          <div className="space-y-4 pt-4 border-t border-outline-variant/30">
            ...
            <Textarea
              ...
              placeholder="Paste JSON array containing clues and content here..."
        ...
        <div className="mt-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="font-headline text-lg font-bold text-on-surface">
                Parsed Questions ({parsedItems.length})
              </h3>
          ...
          <div className="flex items-center justify-end gap-3 pt-6 border-t border-outline-variant/40">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/fill-in-the-blanks-with-clues")}
            ...
            >
              {importMutation.isPending
                ? "Importing..."
                : `Import ${parsedItems.length} Question(s)`}
            </Button>
          </div>
        </div>
      )}
