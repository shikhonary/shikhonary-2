...
import { useApplicationById, useUpdateApplication, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-application"
...
const editApplicationFormSchema = z.object({
  ...
  title: z.string().min(1, "Application topic/title is required"),
...
type EditApplicationFormData = z.infer<typeof editApplicationFormSchema>
...
export function EditApplicationView() {
  ...
  const { data: applicationData, isLoading: isApplicationLoading, isError } = useApplicationById(applicationId)
  ...
  useEffect(() => {
    if (applicationData) {
      reset({
        classId: (applicationData as any).subject?.classSubjects?.[0]?.classId || "",
        subjectId: applicationData.subjectId,
        title: applicationData.title,
        referenceText: applicationData.reference?.join(", ") || "",
        difficulty: applicationData.difficulty as any,
        popularityCount: String(applicationData.popularityCount),
      })
    }
  ...
  if (isApplicationLoading) {
    return (
      <div className="py-24 text-center">
        ...
        <p className="mt-3 font-body-md text-sm text-outline">Loading application details...</p>
      </div>
    )
  }
  ...
  if (isError || !applicationData) {
    return (
      <div className="py-24 text-center max-w-md mx-auto space-y-4">
        <p className="text-xl font-bold text-error">Application Not Found</p>
        <p className="text-sm text-outline">
          The requested application could not be loaded or may have been deleted.
        </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">
                Update prompt content, reference tags, or metadata for Application ID: <span className="font-mono text-outline">{applicationId}</span>
              </p>
            </div>
          </div>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="title" className="text-sm font-bold text-on-surface">
                Application Topic / Prompt / Title <span className="text-red-500">*</span>
              </Label>
              <Textarea
                id="title"
                rows={4}
                placeholder="e.g., Write an application to the Principal of your college requesting a full free studentship, or Write an application for setting up a computer club."
                {...register("title")}
