...
export function TaxPaymentModal({
  ...
  const store = useTaxPaymentModalStore()
  const receiptStore = useTaxReceiptModalStore()
  ...
  const finalPreselected = preselectedTaxPayer !== undefined ? preselectedTaxPayer : store.preselectedTaxPayer
  ...
  const hasFilter = wardId !== "all" || taxPayerSearch.trim().length > 0 || !!finalPreselected
  ...
  const { data: taxPayersData, isLoading: isLoadingTaxPayers } = useQuery({
    ...
    enabled: hasFilter && finalOpen,
  })
  const taxPayers = (taxPayersData?.taxPayers || []).slice(0, 3)

  useEffect(() => {
    if (finalPreselected) {
      setTaxPayerId(finalPreselected.id)
      setAmount(finalPreselected.tax || 0)
      if (finalPreselected.wardId) {
        setWardId(finalPreselected.wardId)
      }
    }
  }, [finalPreselected, finalOpen])

  // Selected TaxPayer Object
  const selectedTaxPayerObj = taxPayerId
    ? taxPayers.find((tp: any) => tp.id === taxPayerId) ||
      (finalPreselected?.id === taxPayerId ? finalPreselected : null)
    : null
