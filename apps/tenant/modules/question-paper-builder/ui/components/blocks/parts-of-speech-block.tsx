...
export function RenderPartsOfSpeechContent({ text, className = "" }: { text?: string; className?: string }) {
  if (!text) return null;
...
const PartsOfSpeechEditableText = ({ 
  text, 
  itemKey, 
  defaultStyle,
  className = ""
  ...
  return (
    <div className="relative group flex-1">
      ...
      >
        <RenderPartsOfSpeechContent text={text} />
      </div>
    </div>
  );
};

export const PartsOfSpeechBlock = ({ item }: { item: any }) => {
  ...
  return (
    <div className={`group relative -mx-4 px-4 hover:bg-muted/10 rounded-lg transition-colors flex flex-col break-inside-avoid ${item.isFirstPartsOfSpeech ? "pt-0.5 pb-0" : "py-0 my-0"}`}>
      {/* Hover Controls */}
      ...
      <AddAlternativeModal
        ...
        primaryQuestionType="PARTS_OF_SPEECH"
