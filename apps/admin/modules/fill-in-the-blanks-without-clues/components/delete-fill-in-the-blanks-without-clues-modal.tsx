...
import { useDeleteFillInTheBlanksWithoutClues, useBulkDeleteFillInTheBlanksWithoutClues } from "../services/use-fill-in-the-blanks-without-clues"
import { useDeleteFillInTheBlanksWithoutCluesModalStore } from "../store/use-delete-fill-in-the-blanks-without-clues-modal-store"

export function DeleteFillInTheBlanksWithoutCluesModal() {
  const { isOpen, fillInTheBlanksId, fillInTheBlanksContent, selectedIds, closeModal } =
    useDeleteFillInTheBlanksWithoutCluesModalStore()

  const [isDeleting, setIsDeleting] = useState(false)
  const deleteMutation = useDeleteFillInTheBlanksWithoutClues()
  const bulkDeleteMutation = useBulkDeleteFillInTheBlanksWithoutClues()
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      } else if (fillInTheBlanksId) {
        await deleteMutation.mutateAsync({ id: fillInTheBlanksId })
      ...
      <DialogContent className="sm:max-w-md bg-white border border-outline-variant rounded-xl shadow-lg">
        <DialogHeader className="flex flex-col gap-2">
          ...
          <DialogTitle className="text-lg font-bold text-on-surface">
            {isBulk ? `Delete ${selectedIds.length} Questions?` : "Delete Question?"}
          </DialogTitle>
          <DialogDescription className="text-sm text-on-surface-variant">
            {isBulk ? (
              ...
              <span>
                Are you sure you want to delete &quot;<strong>{fillInTheBlanksContent || "this question"}</strong>&quot;? This action cannot be undone.
              </span>
            )}
          </DialogDescription>
        </DialogHeader>
