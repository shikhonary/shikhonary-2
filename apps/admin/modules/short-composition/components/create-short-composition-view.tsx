...
const createShortCompositionFormSchema = z.object({
...
type CreateShortCompositionFormData = z.infer<typeof createShortCompositionFormSchema>

export function CreateShortCompositionView() {
  const router = useRouter()
  const createMutation = useCreateShortComposition()
  ...
  } = useForm<CreateShortCompositionFormData>({
    resolver: zodResolver(createShortCompositionFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  const onSubmit = async (data: CreateShortCompositionFormData) => {
    ...
    try {
      ...
      toast.success("Short Composition created successfully.")
      setTimeout(() => {
        router.push("/short-composition")
      }, 500)
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to create Short Composition")
      toast.error(err.message || "Failed to create Short Composition")
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-xs text-on-surface-variant">
                Add a new short composition topic prompt to the question bank.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-outline-variant/30 pt-6">
              ...
              >
                {isSubmitting ? (
                  ...
                  <span>Create Short Composition</span>
                )}
              </Button>
            </div>
          </form>
