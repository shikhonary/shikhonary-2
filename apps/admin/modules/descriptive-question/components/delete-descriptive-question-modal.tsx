...
import { useDeleteDescriptiveQuestion, useBulkDeleteDescriptiveQuestions } from "../services/use-descriptive-question"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function DeleteDescriptiveQuestionModal() {
  const { isOpen, descriptiveQuestionId, questionText, selectedIds, closeModal } =
    useDeleteDescriptiveQuestionModalStore()

  const deleteMutation = useDeleteDescriptiveQuestion()
  const bulkDeleteMutation = useBulkDeleteDescriptiveQuestions()
  const [isDeleting, setIsDeleting] = useState(false)

  const isBulkOpen = isOpen && selectedIds.length > 0
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (isBulkOpen) {
        ...
        toast.success(`Successfully deleted ${count} Descriptive Questions.`)
      } else if (descriptiveQuestionId) {
        await deleteMutation.mutateAsync({ id: descriptiveQuestionId })
        toast.success("Descriptive Question deleted successfully.")
      }
      closeModal()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete Descriptive Question")
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulkOpen ? `Delete ${selectedIds.length} Descriptive Questions?` : "Delete Descriptive Question?"}
            </DialogTitle>
            ...
            {isBulkOpen ? (
              <>
                ...
                <span className="font-bold text-on-surface">
                  {selectedIds.length} selected Descriptive Questions
                </span>
                ? All associated database records will be permanently removed.
              </>
            ) : (
              <>
                Are you sure you want to delete this Descriptive Question:
                <strong className="block mt-2 rounded-lg border border-outline-variant/40 bg-surface-container-lowest p-3 text-xs font-mono text-on-surface-variant line-clamp-2 break-all font-medium">
                  &quot;{questionText || "(ID: " + descriptiveQuestionId + ")"}&quot;
                </strong>
              </>
            )}
          </DialogDescription>
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-700">
          ...
          <p className="leading-snug">
            This action cannot be undone. Associated exam entries and paper setups will be affected.
          </p>
        </div>
