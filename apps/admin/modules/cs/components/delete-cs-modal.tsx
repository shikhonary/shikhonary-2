...
import { useDeleteCs, useBulkDeleteCs } from "../services/use-cs"
...
interface DeleteCsModalProps {
  onSuccess?: () => void
}

export function DeleteCsModal({ onSuccess }: DeleteCsModalProps) {
  const { isOpen, csId, csSnippet, selectedIds, closeModal } =
    useDeleteCsModalStore()

  const deleteMutation = useDeleteCs()
  const bulkDeleteMutation = useBulkDeleteCs()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.deletedCount} CS items.`)
      } else if (csId) {
        await deleteMutation.mutateAsync({ id: csId })
        toast.success("CS question deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete CS")
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} CSs?` : "Delete Creative Scenario Question?"}
            </DialogTitle>
            ...
            {isBulk ? (
              <>
                ...
                <span className="font-bold text-on-surface">
                  {selectedIds.length} selected creative scenario questions
                </span>
                ? All associated sub-questions, reference records, and analytics will be permanently removed.
              </>
            ) : (
              <>
                Are you sure you want to delete the creative scenario question:
