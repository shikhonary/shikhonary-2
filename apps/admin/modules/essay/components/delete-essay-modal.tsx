...
import { useDeleteEssay, useBulkDeleteEssays } from "../services/use-essay"
...
export function DeleteEssayModal({ onSuccess }: DeleteEssayModalProps) {
  const { isOpen, essayId, essayTitle, selectedIds, closeModal } =
    useDeleteEssayModalStore()

  const deleteMutation = useDeleteEssay()
  const bulkDeleteMutation = useBulkDeleteEssays()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.deletedCount} essays.`)
      } else if (essayId) {
        await deleteMutation.mutateAsync({ id: essayId })
        toast.success("Essay deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete Essay")
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} Essays?` : "Delete Essay?"}
            </DialogTitle>
        ...
        <div className="rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 space-y-2">
          {isBulk ? (
            <p className="font-body-md text-sm text-on-surface leading-relaxed">
              ...
              selected essays from the question bank.
            </p>
          ) : (
            <>
              ...
              <p className="font-body-md text-sm font-semibold text-on-surface line-clamp-2">
                &ldquo;{essayTitle || "Untitled Essay"}&rdquo;
              </p>
              {essayId && (
          ...
          <div className="flex items-center gap-2 pt-2 text-[11px] text-amber-700 font-medium">
            ...
            <span>
              If this essay is attached to active exams or question papers, dependent associations may be affected.
            </span>
          </div>
        </div>
          ...
          >
            {isDeleting ? (
              ...
              <>
                ...
                <span>{isBulk ? `Delete ${selectedIds.length} Essays` : "Delete Essay"}</span>
              </>
            )}
          </Button>
        </div>
