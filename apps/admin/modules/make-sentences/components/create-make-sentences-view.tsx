...
export function CreateMakeSentencesView() {
  const router = useRouter()
  const createMutation = useCreateMakeSentences()
  ...
  } = useForm<CreateMakeSentencesFormData>({
    resolver: zodResolver(createMakeSentencesFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: CreateMakeSentencesFormData) => {
    ...
    try {
      ...
      const msg = err.message || "Failed to create word entry"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          <nav className="mb-4 flex items-center space-x-2 text-on-surface-variant">
            ...
            <span className="font-label-sm font-bold text-primary text-xs">Create Word</span>
          </nav>
          <h2 className="mb-2 font-headline-md text-3xl font-extrabold text-primary">
            Create Sentence Making Word
          </h2>
          <p className="font-body-md text-on-surface-variant leading-relaxed text-sm">
            Add a new word (শব্দ দিয়ে বাক্য তৈরি) to the chapter-based question bank.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {isSubmitting ? "Creating..." : "Save Word Entry"}
          </Button>
        </div>
      </form>
    </div>
