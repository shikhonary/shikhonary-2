...
import { useCreatePoem, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-poem"
...
const createPoemFormSchema = z.object({
...
type CreatePoemFormData = z.infer<typeof createPoemFormSchema>

export function CreatePoemView() {
  ...
  } = useForm<CreatePoemFormData>({
    resolver: zodResolver(createPoemFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  const onSubmit = async (data: CreatePoemFormData) => {
    ...
    try {
      ...
      setErrorMessage(err.message || "Failed to create Poem")
      toast.error(err.message || "Failed to create Poem")
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">
                Add a new poem or verse prompt (কবিতা ও কবিতাংশ) with reference citations.
              </p>
            </div>
          </div>
        </CardHeader>
