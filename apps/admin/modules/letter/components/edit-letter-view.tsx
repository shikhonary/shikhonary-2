...
import { useLetterById, useUpdateLetter, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-letter"
...
const editLetterFormSchema = z.object({
...
type EditLetterFormData = z.infer<typeof editLetterFormSchema>

export function EditLetterView() {
  const router = useRouter()
  const params = useParams()
  const letterId = params.id as string

  const { data: letterData, isLoading: isLetterLoading, isError } = useLetterById(letterId)
  ...
  } = useForm<EditLetterFormData>({
    resolver: zodResolver(editLetterFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  useEffect(() => {
    if (letterData) {
      reset({
        classId: (letterData as any).subject?.classSubjects?.[0]?.classId || "",
        subjectId: letterData.subjectId,
        title: letterData.title,
        referenceText: letterData.reference?.join(", ") || "",
        difficulty: letterData.difficulty as any,
        popularityCount: String(letterData.popularityCount),
      })
    }
  }, [letterData, reset])
  ...
  const onSubmit = async (data: EditLetterFormData) => {
    ...
    try {
      ...
      setErrorMessage(err.message || "Failed to update Letter")
  ...
  if (isLetterLoading) {
    return (
      <div className="py-24 text-center">
        ...
        <p className="mt-3 font-body-md text-sm text-outline">Loading letter details...</p>
      </div>
    )
  }
  ...
  if (isError || !letterData) {
    return (
      <div className="py-24 text-center max-w-md mx-auto space-y-4">
        <p className="text-xl font-bold text-error">Letter Not Found</p>
        <p className="text-sm text-outline">
          The requested letter could not be loaded or may have been deleted.
        </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">
                Update prompt content, reference tags, or metadata for Letter ID: <span className="font-mono text-outline">{letterId}</span>
              </p>
            </div>
          </div>
        </CardHeader>
