...
import {
  useShortQuestionById,
  useUpdateShortQuestion,
...
const editShortQuestionFormSchema = z.object({
...
type EditShortQuestionFormData = z.infer<typeof editShortQuestionFormSchema>

export function EditShortQuestionView({ shortQuestionId }: { shortQuestionId?: string }) {
  const router = useRouter()
  const params = useParams()
  const resolvedId = shortQuestionId || (params.id as string)
  const { data: item, isLoading, isError } = useShortQuestionById(resolvedId)
  const updateMutation = useUpdateShortQuestion()
  ...
  } = useForm<EditShortQuestionFormData>({
    resolver: zodResolver(editShortQuestionFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: EditShortQuestionFormData) => {
    ...
    try {
      ...
      toast.success("Short Question updated successfully.")
      setTimeout(() => {
        router.push("/short-questions")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to update Short Question"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
  ...
  if (isError || !item) {
    return (
      <div className="p-8 text-center text-error font-semibold">
        Short Question not found or failed to load.
      </div>
    )
  }
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <p className="font-body-md text-on-surface-variant leading-relaxed text-sm">
            Modify question text, difficulty, or academic assignment.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {isSubmitting ? "Updating..." : "Update Short Question"}
          </Button>
        </div>
      </form>
    </div>
