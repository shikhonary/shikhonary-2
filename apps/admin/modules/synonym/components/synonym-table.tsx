      ...
      {isLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading Synonym Words...</span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-error rounded-xl border border-error/30 bg-error-container/20">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">
              Error loading words. Please try refreshing.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="font-headline-md text-xl font-bold text-on-surface">
              No Synonym Words Found
            </p>
            <p className="font-body-md text-sm text-outline max-w-md">
              Try adjusting filters or add a new word to the bank.
            </p>
          </div>
        </div>
      )}
                    ...
                    <div className="flex-1 space-y-4 min-w-0">
                      ...
                      <div className="rounded-xl border border-secondary/20 bg-secondary-container/10 p-4 text-sm text-on-surface leading-relaxed space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div className={cn(
                            "whitespace-pre-wrap font-bold text-lg text-primary flex items-center gap-2",
                          ...
                          {item.synonymWord && (
                            <div className={cn(
                              "whitespace-pre-wrap font-bold text-base text-emerald-700 flex items-center gap-2 bg-emerald-50 px-3 py-1 rounded-lg border border-emerald-200",
                              /[\u0980-\u09FF]/.test(item.synonymWord) && "font-solaiman text-lg"
                            )}>
                              <span className="text-xs font-medium text-emerald-600">সমার্থক শব্দ:</span>
                              <RenderMath text={item.synonymWord} isMath={true} />
                            </div>
                          )}
                        </div>
                        ...
                        {Array.isArray(item.synonyms) && item.synonyms.length > 0 && (
                          <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-secondary/20">
                            <span className="text-xs font-medium text-on-surface-variant">অন্যান্য সমার্থক শব্দ:</span>
                            {item.synonyms.map((syn, synIdx) => (
                              <span
                                key={synIdx}
                                className="px-2 py-0.5 bg-emerald-100/60 text-emerald-800 rounded-md text-xs font-semibold border border-emerald-200"
                              >
                                {syn}
                              </span>
                            ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border border-outline-variant bg-surface-container-low rounded-xl p-4">
          <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-start">
            <p className="font-body-md text-xs sm:text-sm text-on-surface-variant">
              Showing <span className="font-bold">{displayStart}-{displayEnd}</span> of <span className="font-bold">{totalItems}</span> words
            </p>
            {onLimitChange && (
              <div className="flex items-center gap-2">
