...
import { useDeleteChangingSentence, useBulkDeleteChangingSentences } from "../services/use-changing-sentence"
...
interface DeleteChangingSentenceModalProps {
  onSuccess?: () => void
}

export function DeleteChangingSentenceModal({ onSuccess }: DeleteChangingSentenceModalProps = {}) {
  const { isOpen, targetId, targetSnippet, selectedIds, closeModal } =
    useDeleteChangingSentenceModalStore()

  const deleteMutation = useDeleteChangingSentence()
  const bulkDeleteMutation = useBulkDeleteChangingSentences()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.count ?? selectedIds.length} questions.`)
      } else if (targetId) {
        await deleteMutation.mutateAsync({ id: targetId })
        toast.success("Changing Sentences question deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} Changing Sentences?` : "Delete Changing Sentence?"}
            </DialogTitle>
            ...
            {isBulk ? (
              <>
                ...
                <span className="font-bold text-on-surface">
                  {selectedIds.length} selected changing sentences
                </span>
                ? All associated directives, options, reference records, and analytics will be permanently removed.
              </>
            ) : (
              <>
                Are you sure you want to delete the changing sentence question:
                <strong className="block mt-2 rounded-lg border border-outline-variant/40 bg-surface-container-lowest p-3 text-xs font-mono text-on-surface-variant line-clamp-2 break-all font-medium">
                  &quot;{targetSnippet || "(No Prompt / Options - ID: " + targetId + ")"}&quot;
                </strong>
              </>
            )}
          </DialogDescription>
