...
import { useImportParagraphs, useAcademicClassesForSelection, useSubjectsForSelection, useChaptersForSelection } from "../services/use-paragraph"
...
const sampleJsonTemplate = `[
  {
    "name": "Passage: A computer is an electronic device that processes data, storing and retrieving information according to instructions.",
...
interface EditableParagraphCardProps {
  index: number
  item: any
...
function EditableParagraphCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableParagraphCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2 border-l border-outline-variant/60 pl-2">
            ...
            <Button
              ...
              size="icon-xs"
              onClick={onDelete}
              title="Delete Paragraph from import"
      ...
      <div className="p-5 space-y-4">
        {/* Name / Text */}
        <EditableField
          label="Paragraph / Passage Text"
          value={item.name || ""}
          multiline
          placeholder="Enter passage text..."
          onSave={(newVal) => onChange({ ...item, name: newVal })}
        />
...
export function ImportParagraphView() {
  const router = useRouter()
  const importMutation = useImportParagraphs()
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        if (!item.name || typeof item.name !== "string" || !item.name.trim()) {
          errors.push(`Item #${itemNum}: Missing or invalid 'name' text.`)
        }
        if (!subjectId) {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Paragraph items."}`)
    }
  }
  ...
  const handleDeleteParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.info(`Removed paragraph #${idx + 1} from import list.`)
  }
  ...
  const handleDuplicateParsedItem = (idx: number) => {
    ...
    syncParsedItemsToText(next)
    toast.success(`Duplicated paragraph #${idx + 1}.`)
  }

  const handleAddNewQuestionCard = () => {
    const newParagraph = {
      name: "New Passage Text...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
    const next = [...parsedItems, newParagraph]
    syncParsedItemsToText(next)
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ paragraphs: payload })
      toast.success(`Successfully imported ${res.importedCount} Paragraphs!`)
      setTimeout(() => {
        router.push("/paragraphs")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import paragraphs.")
    }
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload paragraphs or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Chapter is optional. Paragraph items will be assigned subject and chapter from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-800 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {parsedItems.length} paragraph(s) parsed successfully.
                  </p>
                </div>
              </div>
              <Button
          ...
          {parsedItems.length > 0 && (
            <div className="border-t border-outline-variant/30 pt-6 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-headline-md text-base font-bold text-on-surface flex items-center gap-2">
                    ...
                    Card Preview ({parsedItems.length} Paragraphs)
                  </h4>
                </div>
              ...
              {/* Grid of Paragraph Preview Cards */}
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/paragraphs")}
            ...
            >
              ...
              <span>
                {importMutation.isPending
                  ? "Importing..."
                  : `Import ${parsedItems.length} Paragraphs`}
              </span>
            </Button>
          </div>
        </CardContent>
