...
import { useDeleteSummary, useBulkDeleteSummaries } from "../services/use-summary"
...
export function DeleteSummaryModal({ onSuccess }: DeleteSummaryModalProps) {
  const { isOpen, summaryId, summaryTitle, selectedIds, closeModal } =
    useDeleteSummaryModalStore()

  const deleteMutation = useDeleteSummary()
  const bulkDeleteMutation = useBulkDeleteSummaries()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.deletedCount} summaries.`)
      } else if (summaryId) {
        await deleteMutation.mutateAsync({ id: summaryId })
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} Summaries?` : "Delete Summary?"}
            </DialogTitle>
        ...
        <div className="rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 space-y-2">
          {isBulk ? (
            <p className="font-body-md text-sm text-on-surface leading-relaxed">
              ...
              selected summaries from the question bank.
            </p>
          ) : (
            <>
              ...
              <p className="font-body-md text-sm font-semibold text-on-surface line-clamp-2">
                &ldquo;{summaryTitle || "Untitled Summary"}&rdquo;
              </p>
              {summaryId && (
          ...
          <div className="flex items-center gap-2 pt-2 text-[11px] text-amber-700 font-medium">
            ...
            <span>
              If this summary is attached to active exams or papers, dependent associations may be affected.
            </span>
          </div>
        </div>
          ...
          >
            {isDeleting ? (
              ...
              <>
                ...
                <span>{isBulk ? `Delete ${selectedIds.length} Summaries` : "Delete Summary"}</span>
              </>
            )}
          </Button>
        </div>
