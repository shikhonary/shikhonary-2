...
interface CitizenApplicationFilterProps {
  wards: any[]
  setCurrentPage: (page: number) => void
}

export function CitizenApplicationFilter({
  wards,
  setCurrentPage,
}: CitizenApplicationFilterProps) {
  ...
  const setStatusFilter = (val: string) => setSearchParams({ status: val })
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-border/60 bg-card/80 backdrop-blur-xs p-3 sm:p-4 font-body">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <Input
            type="text"
            placeholder="আবেদনকারী, মোবাইল বা NID খুঁজুন..."
            value={searchQuery}
            onChange={(e) => {
        ...
        <Drawer>
          <DrawerTrigger asChild>
            ...
            >
              ...
              {(wardFilter !== "all" || statusFilter !== "all" || sortFilter !== "newest") && (
                <span className="flex size-4.5 items-center justify-center rounded-full bg-primary text-primary-foreground text-[10px] font-bold">
                  {(wardFilter !== "all" ? 1 : 0) + (statusFilter !== "all" ? 1 : 0) + (sortFilter !== "newest" ? 1 : 0)}
                </span>
              )}
            </Button>
          </DrawerTrigger>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <DrawerDescription className="font-body text-xs text-primary-foreground/90 mt-0.5">
                    আবেদনের তালিকা ওয়ার্ড, অবস্থা ও সর্টিং অনুযায়ী ফিল্টার করুন
                  </DrawerDescription>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <SelectContent className="bg-popover border border-border shadow-xl rounded-xl text-popover-foreground">
                      ...
                      <SelectItem value="name_asc">নাম (ক - ঞ)</SelectItem>
                      <SelectItem value="name_desc">নাম (ঞ - ক)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
            ...
            >
              <SelectTrigger className="w-full rounded-xl border border-border bg-muted/30 py-2.5 px-3.5 font-body text-xs text-foreground h-10 justify-between hover:bg-muted/50 transition-all">
                <div className="flex items-center gap-2 truncate">
                  ...
                  <SelectValue placeholder="ক্রম নির্বাচন" />
                </div>
              </SelectTrigger>
      ...
      {(Boolean(searchQuery.trim()) || wardFilter !== "all" || statusFilter !== "all" || sortFilter !== "newest") && (
        <div className="flex flex-col gap-2.5 rounded-xl border border-border/40 bg-card/40 p-3 text-xs sm:flex-row sm:items-center sm:justify-between sm:bg-transparent sm:border-0 sm:p-0 sm:px-1">
          ...
          <Button
            ...
            size="sm"
            onClick={resetFilters}
            className="h-7 text-[11px] sm:text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded-lg shrink-0 cursor-pointer"
          >
