...
import { useCsById, useUpdateCs, useSubjectsForSelection, useChaptersForSelection, useAcademicClassesForSelection } from "../services/use-cs"
...
const editCsFormSchema = z.object({
...
type EditCsFormData = z.infer<typeof editCsFormSchema>

interface EditCsViewProps {
  id?: string
}

export function EditCsView({ id }: EditCsViewProps = {}) {
  const router = useRouter()
  const params = useParams()
  const targetId = (id || (params?.id as string) || "") as string
  const { data: csData, isLoading: isCsLoading, isError } = useCsById(targetId)
  const cs = csData as any
  const updateMutation = useUpdateCs()
  ...
  } = useForm<EditCsFormData>({
    resolver: zodResolver(editCsFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  useEffect(() => {
    if (cs) {
      const refText = Array.isArray(cs.reference) ? cs.reference.join(", ") : ""
      const classId = cs.subject?.classSubjects?.[0]?.academicClassId || ""

      reset({
        classId,
        subjectId: cs.subjectId || "",
        chapterId: cs.chapterId || "",
        questionA: cs.questionA || "",
        questionB: cs.questionB || "",
        referenceText: refText,
        difficulty: (cs.difficulty as any) || QUESTION_DIFFICULTY.MEDIUM,
        questionTypeId: cs.questionTypeId || "",
  ...
  const isSubmitting = updateMutation.isPending || isFormSubmitting || isCsLoading

  const onSubmit = async (data: EditCsFormData) => {
    ...
    try {
      ...
      toast.success("Creative Scenario Question updated successfully.")
      setTimeout(() => {
        router.push("/cs")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to update Creative Scenario Question"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          <nav className="mb-3 flex items-center space-x-2 text-on-surface-variant">
            ...
            <span className="font-label-sm text-xs font-bold text-primary">Edit CS</span>
          </nav>
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Edit Creative Scenario Question
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Update sub-questions A & B, difficulty level, reference tags, or status for this creative scenario question.
          </p>
        </div>
      </div>
      ...
      {isCsLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading CS details...</span>
          </div>
        </div>
      )}

      {/* Error State */}
      {(isError || (!isCsLoading && !cs)) && (
        <div className="py-16 text-center text-error rounded-xl border border-error/30 bg-error-container/20">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">
              CS Question Not Found.
            </span>
            <Link href="/cs" className="text-primary hover:underline text-xs font-bold mt-2">
              Back to CS List
            </Link>
          </div>
        </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-outline-variant/40 pt-6 sm:pt-8 mt-6">
                  <div className="flex items-center justify-center sm:justify-start space-x-2 text-on-surface-variant">
                    ...
                    <span className="text-[12px]">ID: {targetId}</span>
                  </div>
                  <div className="flex flex-col-reverse sm:flex-row w-full sm:w-auto items-stretch sm:items-center gap-3 sm:gap-4">
                    ...
                    >
                      ...
                      <span>{isSubmitting ? "Updating..." : "Update CS Question"}</span>
                    </Button>
                  </div>
                </div>
              </form>
