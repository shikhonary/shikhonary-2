...
import { QuestionBankCqCard, QuestionBankCqItem } from "./question-bank-cq-card"

const MAX_PAGE_BUTTONS = 5

interface QuestionBankCqListViewProps {
  subjectId: string
  initialChapterId?: string
}

export function QuestionBankCqListView({
  subjectId,
  initialChapterId,
}: QuestionBankCqListViewProps) {
  ...
  // Fetch chapters for chapter selector
  const chaptersQuery = useQuery(
    trpc.questionBank.byChapter.queryOptions({ subjectId })
  )

  // Fetch board years for CQ board years selector
  ...
  const cqData = cqsQuery.data
  const chapters = chaptersQuery.data ?? []
  const boardYears = boardYearsQuery.data ?? []
  const isLoading = cqsQuery.isLoading
  const isRefetching = cqsQuery.isFetching && !cqsQuery.isLoading
  ...
  const totalItems = cqData?.totalItems ?? 0
  ...
  const pageButtons = buildPageButtons(page, cqData?.totalPages ?? 1)
    ...
    <div className="space-y-5 font-solaiman">
      ...
      <div className="flex items-center gap-3 rounded-xl border border-outline-variant/30 bg-surface-container-low p-3 sm:p-4">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <Input
            type="text"
            placeholder="Search creative questions..."
            value={searchQuery}
            onChange={(e) => {
          ...
          <DrawerContent className="p-6 space-y-5 bg-white border-t border-outline-variant/40 max-h-[90vh] overflow-y-auto">
            <DrawerHeader className="p-0 text-left">
              ...
              <DrawerDescription className="text-xs text-on-surface-variant">
                Select filters, sorting, and page options to refine CQ records.
              </DrawerDescription>
            </DrawerHeader>
      ...
      {cqData && (
        <div className="flex items-center justify-between px-1 text-xs text-on-surface-variant font-solaiman">
          <div className="flex items-center gap-1.5">
            ...
            <span>
              <strong className="font-bold text-on-background">{cqData.totalItems}</strong>{" "}
              creative question{cqData.totalItems !== 1 ? "s" : ""}
            </span>
            {isRefetching && (
      ...
      {isLoading ? (
      ...
      ) : cqData?.items && cqData.items.length > 0 ? (
        <>
          {/* CQ cards list */}
          <div className="space-y-4">
            {cqData.items.map((item, idx) => (
              <QuestionBankCqCard
                key={item.id}
                item={item as unknown as QuestionBankCqItem}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border border-outline-variant bg-surface-container-low rounded-xl p-4 mt-6 font-solaiman">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  disabled={page >= (cqData?.totalPages ?? 1)}
                  onClick={() => setPage(Math.min(cqData?.totalPages ?? 1, page + 1))}
                  className="h-9 w-9 rounded-lg p-0 text-xs cursor-pointer border border-outline-variant bg-white transition-colors hover:bg-surface-container-high disabled:opacity-30"
        ...
        <div className="flex min-h-[240px] flex-col items-center justify-center rounded-xl border border-outline-variant/40 bg-surface-container-lowest p-8 text-center font-solaiman">
          <span className="material-symbols-outlined mb-3 text-5xl text-outline">description</span>
          <h3 className="mb-1 text-base font-bold text-on-background">No creative questions found</h3>
