...
import { useDeleteAcademicYear } from "../services/use-academic-year"
...
export function DeleteAcademicYearModal() {
  const { isOpen, yearId, yearName, closeModal } =
    useDeleteAcademicYearModalStore()
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(
        `Academic year "${yearName || "Academic Year"}" deleted successfully.`
      )
      closeModal()
    } catch (err: any) {
      ...
      >
        ...
        <DialogHeader className="pt-1">
          <DialogDescription className="font-body-md text-sm leading-relaxed text-on-surface-variant">
            ...
            ? All subjects, classes, chapters, and curriculum cycles mapped to this year will lose references.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-700">
          ...
          <p className="leading-snug">
            This process is permanent and cannot be undone. Please ensure no active student sessions depend on this academic year.
          </p>
        </div>
