...
export function DeleteCqModal({ onSuccess }: DeleteCqModalProps) {
  const { isOpen, cqId, cqQuestion, selectedIds, closeModal } =
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success("Creative Question deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
          ...
          <DialogDescription className="font-body-md text-sm leading-relaxed text-on-surface-variant">
            {isBulk ? (
              <>
                ...
                ? All associated sub-questions, answer keys, explanations, and attachments will be permanently removed.
              </>
            ) : (
              <>
