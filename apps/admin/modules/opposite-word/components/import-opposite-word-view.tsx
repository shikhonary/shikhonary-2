...
interface EditableOppositeWordCardProps {
  index: number
  item: any
...
function EditableOppositeWordCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableOppositeWordCardProps) {
  ...
  const handleSaveOppositeWords = (val: string) => {
    const words = val
      .split(",")
      .map((w) => w.trim())
      .filter(Boolean)
  ...
  const oppositeWordsString = Array.isArray(item.oppositeWords) ? item.oppositeWords.join(", ") : ""
    ...
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Word Text */}
          <EditableField
            label="Primary Word (মূল শব্দ)"
            value={item.word || item.question || item.name || item.title || ""}
            placeholder="Enter word text (e.g. আকাশ, আলো)..."
            onSave={(newVal) => onChange({ ...item, word: newVal })}
          />

          {/* Opposite Word */}
          <EditableField
            label="Opposite Word (বিপরীত শব্দ)"
            value={item.oppositeWord || ""}
            placeholder="Enter opposite word (e.g. পাতাল, অন্ধকার)..."
            isMath={false}
            onSave={(newVal) => onChange({ ...item, oppositeWord: newVal })}
          />
        </div>
        ...
        <div className="border-t border-outline-variant/30 pt-3">
          <EditableField
            label="Alternative Antonyms (বিকল্প বিপরীত শব্দসমূহ - Comma-separated)"
            value={oppositeWordsString}
            placeholder="e.g. ধরা, ভূমি..."
            isMath={false}
            onSave={handleSaveOppositeWords}
          />
        </div>
...
export function ImportOppositeWordView() {
  const router = useRouter()
  const importMutation = useImportOppositeWord()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      itemsArray.forEach((item: any, idx: number) => {
        ...
        const rawOpposites = Array.isArray(item.oppositeWords)
          ? item.oppositeWords.map(String)
          : []
        ...
        validated.push({
          ...
          oppositeWord: item.oppositeWord ? String(item.oppositeWord).trim() : null,
          oppositeWords: rawOpposites,
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Opposite Word items."}`)
    }
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to bulk import opposite words.")
    }
  }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Opposite Words from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload chapter-based opposite words (বিপরীত শব্দ) or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-6">
        ...
        >
          <Link href="/opposite-words">Cancel</Link>
        </Button>
        ...
        >
          {importMutation.isPending ? (
            ...
            <>
              ...
              <span>Import {parsedItems.length} Opposite Word Entries</span>
            </>
          )}
        </Button>
      </div>
