...
import { useDeletePunctuation, useBulkDeletePunctuation } from "../services/use-punctuation"
import { useDeletePunctuationModalStore } from "../store/use-delete-punctuation-modal-store"

export function DeletePunctuationModal() {
  const { isOpen, punctuationId, punctuationContent, selectedIds, closeModal } =
    useDeletePunctuationModalStore()

  const [isDeleting, setIsDeleting] = useState(false)
  const deleteMutation = useDeletePunctuation()
  const bulkDeleteMutation = useBulkDeletePunctuation()
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${selectedIds.length} Punctuation items.`)
      } else if (punctuationId) {
        await deleteMutation.mutateAsync({ id: punctuationId })
        toast.success("Punctuation item deleted successfully.")
      }
      ...
      <DialogContent className="sm:max-w-md bg-white border border-outline-variant rounded-xl shadow-lg">
        <DialogHeader className="flex flex-col gap-2">
          ...
          <DialogTitle className="text-lg font-bold text-on-surface">
            {isBulk ? `Delete ${selectedIds.length} Questions?` : "Delete Punctuation Question?"}
          </DialogTitle>
          <DialogDescription className="text-sm text-on-surface-variant">
            {isBulk ? (
              ...
              <span>
                Are you sure you want to delete &quot;<strong>{punctuationContent || "this question"}</strong>&quot;? This action cannot be undone.
              </span>
            )}
          </DialogDescription>
        </DialogHeader>
