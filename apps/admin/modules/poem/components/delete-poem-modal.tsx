...
import { useDeletePoem, useBulkDeletePoems } from "../services/use-poem"
...
export function DeletePoemModal({ onSuccess }: DeletePoemModalProps) {
  const { isOpen, poemId, poemTitle, selectedIds, closeModal } =
    useDeletePoemModalStore()

  const deleteMutation = useDeletePoem()
  const bulkDeleteMutation = useBulkDeletePoems()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.deletedCount} poems.`)
      } else if (poemId) {
        await deleteMutation.mutateAsync({ id: poemId })
        toast.success("Poem deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete Poem")
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} Poems?` : "Delete Poem?"}
            </DialogTitle>
        ...
        <div className="rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 space-y-2">
          {isBulk ? (
            <p className="font-body-md text-sm text-on-surface leading-relaxed">
              ...
              selected poems from the question bank.
            </p>
          ) : (
            <>
              ...
              <p className="font-body-md text-sm font-semibold text-on-surface line-clamp-2">
                &ldquo;{poemTitle || "Untitled Poem"}&rdquo;
              </p>
              {poemId && (
          ...
          <div className="flex items-center gap-2 pt-2 text-[11px] text-amber-700 font-medium">
            ...
            <span>
              If this poem is attached to active exams or papers, dependent associations may be affected.
            </span>
          </div>
        </div>
          ...
          >
            {isDeleting ? (
              ...
              <>
                ...
                <span>{isBulk ? `Delete ${selectedIds.length} Poems` : "Delete Poem"}</span>
              </>
            )}
          </Button>
        </div>
