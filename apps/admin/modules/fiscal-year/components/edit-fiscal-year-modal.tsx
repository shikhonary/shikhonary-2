...
import { useFiscalYearById, useUpdateFiscalYear } from "../services/use-fiscal-year"
import { useEditFiscalYearModalStore } from "../store/use-edit-fiscal-year-modal-store"

export function EditFiscalYearModal() {
  const { isOpen, yearId, closeModal } = useEditFiscalYearModalStore()
  const { data: year, isLoading, isError } = useFiscalYearById(yearId ?? "", isOpen && Boolean(yearId))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.message || "Failed to update fiscal year"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
        ...
        <CardHeader className="border-b border-outline-variant/40 bg-surface-container-lowest p-6 flex flex-row items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-body-md text-on-surface-variant mt-0.5">
              Modify financial year label, duration, and active status.
            </p>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-6 max-h-[calc(100vh-12rem)] overflow-y-auto">
          {isLoading ? (
            <div className="flex items-center justify-center p-12 text-on-surface-variant">
              ...
              <span className="ml-3 font-body-md text-sm">Loading fiscal year details...</span>
            </div>
          ) : isError ? (
            <div className="p-8 text-center text-error">
              <p className="font-body-md font-medium">Failed to load fiscal year details.</p>
