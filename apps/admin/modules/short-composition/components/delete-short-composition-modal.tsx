...
import { useDeleteShortComposition, useBulkDeleteShortCompositions } from "../services/use-short-composition"
...
interface DeleteShortCompositionModalProps {
  onSuccess?: () => void
}

export function DeleteShortCompositionModal({ onSuccess }: DeleteShortCompositionModalProps) {
  const { isOpen, shortCompositionId, shortCompositionTitle, selectedIds, closeModal } =
    useDeleteShortCompositionModalStore()

  const deleteMutation = useDeleteShortComposition()
  const bulkDeleteMutation = useBulkDeleteShortCompositions()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.count} short compositions.`)
      } else if (shortCompositionId) {
        await deleteMutation.mutateAsync({ id: shortCompositionId })
        toast.success("Short Composition deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete Short Composition")
    }
  }
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} Short Compositions?` : "Delete Short Composition?"}
            </DialogTitle>
        ...
        <div className="rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 space-y-2">
          ...
          <div className="flex items-center gap-2 pt-2 text-[11px] text-amber-700 font-medium">
            ...
            <span>
              If this question is attached to active exams or question papers, dependent associations may be affected.
            </span>
          </div>
        </div>
          ...
          >
            {isDeleting ? (
              ...
              <>
                ...
                <span>{isBulk ? `Delete ${selectedIds.length} Short Compositions` : "Delete"}</span>
              </>
            )}
          </Button>
        </div>
