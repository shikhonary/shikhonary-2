...
export function RenderChangingSentenceContent({ text, className = "" }: { text?: string; className?: string }) {
  if (!text) return null;

  // Split by <u>...</u> tags first if present
    ...
    <span className={className}>
      {parts.map((part, index) => {
        if (/^<u>(.*?)<\/u>$/i.test(part)) {
          ...
          return (
            <u key={index} className="underline underline-offset-4 decoration-1 font-semibold">
              <RenderMath text={word} />
            </u>
          );
        }
...
const ChangingSentenceEditableText = ({ 
  text, 
  itemKey, 
  defaultStyle,
  className = ""
  ...
  return (
    <div className="relative group flex-1">
      ...
      >
        <RenderChangingSentenceContent text={text} />
      </div>
    </div>
  );
};

export const ChangingSentenceBlock = ({ item }: { item: any }) => {
  ...
  const getQuestionMark = (index: number, defaultMark: number) => {
    if (markDist) {
      if (Array.isArray(markDist)) {
        ...
        const alphaKeys = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"];
        const upperAlphaKeys = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"];
        const bengaliKeys = ["ক", "খ", "গ", "ঘ", "ঙ", "চ", "ছ", "জ", "ঝ", "ঞ"];
  ...
  // Content or Options check: either passage content OR discrete sentence options
  const hasPassageContent = Boolean(data.content && typeof data.content === "string" && data.content.trim());
  const optionsList: string[] = Array.isArray(data.options) ? data.options : [];
  const hasOptionsList = optionsList.length > 0;

  return (
    <div className={`group relative -mx-4 px-4 hover:bg-muted/10 rounded-lg transition-colors flex flex-col break-inside-avoid ${item.isFirstChangingSentence ? "pt-0.5 pb-0" : "py-0 my-0"}`}>
      {/* Hover Controls */}
      ...
      <div className="flex justify-between items-start gap-2 w-full">
        <div className="flex gap-1 flex-1 relative flex-col">
          <div className="flex gap-2 items-start w-full">
            ...
            {hasPassageContent && item.totalQuestions > 1 && renderSubQuestionLabel(label)}

            <div className="flex-1 w-full min-w-0">
              {/* Conditional rendering: either passage content OR discrete sentence options */}
              {hasPassageContent ? (
                <ChangingSentenceEditableText 
                  text={data.content || ""}
                ...
                <div className="space-y-1">
                  {optionsList.map((opt, optIdx) => {
                    ...
                    const optLabel: string = match?.[1] || subLabels[optIdx] || String(optIdx + 1);
                    const optText: string = (match?.[2] !== undefined ? match[2] : opt) || "";

                    return (
                      <div key={optIdx} className="flex gap-2 items-start w-full">
                        {renderSubQuestionLabel(optLabel)}
                        <div className="flex-1 min-w-0">
                          <ChangingSentenceEditableText 
                            text={optText}
                            itemKey={`${item.id}-option-${optIdx}`}
                            defaultStyle={questionStyle}
                            className="m-0 w-full leading-relaxed"
                          />
                        </div>
                      </div>
                    );
      ...
      <AddAlternativeModal
        ...
        primaryQuestionType="CHANGING_SENTENCES"
