...
import {
  useDescriptiveQuestionById,
  useUpdateDescriptiveQuestion,
...
const editDescriptiveQuestionFormSchema = z.object({
...
type EditDescriptiveQuestionFormData = z.infer<typeof editDescriptiveQuestionFormSchema>

export function EditDescriptiveQuestionView({ descriptiveQuestionId }: { descriptiveQuestionId?: string }) {
  const router = useRouter()
  const params = useParams()
  const resolvedId = descriptiveQuestionId || (params.id as string)
  const { data: item, isLoading, isError } = useDescriptiveQuestionById(resolvedId)
  const updateMutation = useUpdateDescriptiveQuestion()
  ...
  } = useForm<EditDescriptiveQuestionFormData>({
    resolver: zodResolver(editDescriptiveQuestionFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: EditDescriptiveQuestionFormData) => {
    ...
    try {
      ...
      toast.success("Descriptive Question updated successfully.")
      setTimeout(() => {
        router.push("/descriptive-questions")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to update Descriptive Question"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
  ...
  if (isError || !item) {
    return (
      <div className="p-8 text-center text-error font-semibold">
        Descriptive Question not found or failed to load.
      </div>
    )
  }
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <p className="font-body-md text-on-surface-variant leading-relaxed text-sm">
            Modify question text, answer, difficulty, or academic assignment.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {isSubmitting ? "Updating..." : "Update Descriptive Question"}
          </Button>
        </div>
      </form>
    </div>
