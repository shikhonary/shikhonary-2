...
import { useDeleteMakeSentences, useBulkDeleteMakeSentences } from "../services/use-make-sentences"

export function DeleteMakeSentencesModal() {
  const { isOpen, makeSentencesId, wordText, selectedIds, closeModal } = useDeleteMakeSentencesModalStore()
  const deleteMutation = useDeleteMakeSentences()
  const bulkDeleteMutation = useBulkDeleteMakeSentences()
  ...
  const handleDelete = async () => {
    ...
    } else if (makeSentencesId) {
      deleteMutation.mutate(
        { id: makeSentencesId },
        {
          onSuccess: () => {
            toast.success("Successfully deleted word entry")
            closeModal()
          },
          onError: (err) => {
            toast.error(err.message || "Failed to delete word entry")
          },
        }
      )
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-bold text-on-surface">
                {isBulk ? `Delete ${selectedIds.length} Words` : "Delete Word Entry"}
              </DialogTitle>
