...
interface ChangingSentenceTableProps {
  items: ChangingSentenceItem[]
  isLoading: boolean
  isError: boolean
...
/**
 * Helper to render bracketed directives styled with accent highlighting
 */
      ...
      {parts.map((part, index) => {
        if (/^\([a-zA-Z\s\/-]+\)$/.test(part)) {
          return (
            <span
              key={index}
              className="inline-block px-1.5 py-0.5 mx-0.5 rounded font-bold font-mono text-xs bg-indigo-500/10 text-indigo-700 border border-indigo-500/20"
            >
              {part}
            </span>
          )
...
export function ChangingSentenceTable({
  items,
  isLoading,
  isError,
  onDelete,
      ...
      {isLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading Changing Sentences...</span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-error rounded-xl border border-error/30 bg-error-container/20">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">
              Error loading Changing Sentences. Please try refreshing.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="font-body-md text-sm text-outline max-w-md">
              Try adjusting filters or add a new changing sentences question to the bank.
            </p>
          </div>
        </div>
      )}
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                      ...
                      {item.content && (
                        <div className="rounded-xl border border-secondary/20 bg-secondary-container/10 p-4 text-sm text-on-surface leading-relaxed">
                          <div className="whitespace-pre-wrap font-medium">
                            <RenderDirectiveHighlights text={item.content} />
                          </div>
                        </div>
                      )}

                      {/* Discrete Sentence Options List */}
                      {item.options && item.options.length > 0 && (
                        <div className="space-y-1.5 rounded-xl border border-outline-variant/40 bg-surface-container-low/50 p-4 text-xs sm:text-sm">
                          <p className="text-[11px] font-bold uppercase tracking-wider text-outline mb-2">
                            Sentences with Directives ({item.options.length})
                          </p>
                          <div className="space-y-1">
                            {item.options.map((opt, optIdx) => (
                              <div key={optIdx} className="text-on-surface font-medium leading-relaxed">
                                <RenderDirectiveHighlights text={opt} />
                              </div>
                            ))}
                          </div>
                        </div>
                  ...
                  <div className="flex md:flex-col justify-end items-center gap-2 shrink-0 border-t md:border-t-0 border-outline-variant/40 pt-3 md:pt-0 w-full md:w-auto">
                    <Link
                      href={`/changing-sentences/${item.id}/edit`}
