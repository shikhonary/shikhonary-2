...
const editGenderChangeFormSchema = z.object({
...
type EditGenderChangeFormData = z.infer<typeof editGenderChangeFormSchema>

interface EditGenderChangeViewProps {
  id: string
}

export function EditGenderChangeView({ id }: EditGenderChangeViewProps) {
  const router = useRouter()
  const { data: item, isLoading, isError } = useGenderChangeById(id)
  const updateMutation = useUpdateGenderChange()
  ...
  } = useForm<EditGenderChangeFormData>({
    resolver: zodResolver(editGenderChangeFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: EditGenderChangeFormData) => {
    ...
    try {
      ...
      toast.success("Gender Change entry updated successfully.")
      setTimeout(() => {
        router.push("/gender-change")
      }, 1000)
    } catch (err: any) {
      const msg = err.message || "Failed to update gender change entry"
      setErrorMessage(msg)
      toast.error(msg)
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="py-20 text-center text-outline flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-sm font-medium">Loading gender change entry details...</span>
      </div>
    )
  }
  ...
  if (isError || !item) {
    return (
      <div className="py-20 text-center text-error border border-error/30 rounded-xl bg-error-container/20">
        <p className="font-bold text-base">Error loading gender change entry</p>
        <p className="text-xs mt-1">The requested entry with ID &quot;{id}&quot; was not found.</p>
        <Link href="/gender-change" className="mt-4 inline-block">
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mb-2 font-headline-md text-3xl font-extrabold text-primary">
            Edit Gender Change Entry
          </h2>
          <p className="font-body-md text-on-surface-variant leading-relaxed text-sm">
            Modify the word details, converted gender form, context, difficulty, or reference.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {isSubmitting ? "Saving..." : "Update Gender Entry"}
          </Button>
        </div>
      </form>
    </div>
