...
interface PartsOfSpeechTableProps {
  items: PartsOfSpeechItem[]
  isLoading: boolean
  isError: boolean
...
/**
 * Helper to render text with <u> tags styled with clean underline and amber badge accent
 */
function RenderContentWithUnderline({ text }: { text: string }) {
  if (!text) return null

  // Split by <u>...</u> tags to render underlined words safely
...
export function PartsOfSpeechTable({
  items,
  isLoading,
  isError,
  onDelete,
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {isLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading Parts of Speech...</span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-error rounded-xl border border-error/30 bg-error-container/20">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">
              Error loading Parts of Speech. Please try refreshing.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="font-body-md text-sm text-outline max-w-md">
              Try adjusting filters or add a new parts of speech question to the bank.
            </p>
          </div>
        </div>
      )}

      {/* CARD VIEW LAYOUT (Matches Paragraph Card Layout) */}
      {!isLoading && !isError && items.length > 0 && (
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                    ...
                    <div className="flex-1 space-y-4 min-w-0">
                      ...
                      <div className="rounded-xl border border-secondary/20 bg-secondary-container/10 p-4 text-sm text-on-surface leading-relaxed">
                        <div className="whitespace-pre-wrap font-medium">
                          <RenderContentWithUnderline text={item.content} />
                        </div>
                      </div>
                  ...
                  <div className="flex md:flex-col justify-end items-center gap-2 shrink-0 border-t md:border-t-0 border-outline-variant/40 pt-3 md:pt-0 w-full md:w-auto">
                    <Link
                      href={`/parts-of-speech/${item.id}/edit`}
      ...
      {/* Pagination Footer (Matches Paragraph Pagination exactly) */}
