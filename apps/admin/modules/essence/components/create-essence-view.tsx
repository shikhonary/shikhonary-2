...
import { useCreateEssence, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-essence"
...
const createEssenceFormSchema = z.object({
...
type CreateEssenceFormData = z.infer<typeof createEssenceFormSchema>

export function CreateEssenceView() {
  const router = useRouter()
  const createMutation = useCreateEssence()
  ...
  } = useForm<CreateEssenceFormData>({
    resolver: zodResolver(createEssenceFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  const onSubmit = async (data: CreateEssenceFormData) => {
    ...
    try {
      ...
      toast.success("Essence created successfully.")
      setTimeout(() => {
        router.push("/essences")
      }, 500)
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to create Essence")
      toast.error(err.message || "Failed to create Essence")
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="font-headline-md text-xl font-bold text-on-surface">
                Create New Essence / Substance
              </CardTitle>
              <p className="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">
                Add a new poem or passage for essence / gist extraction (সারমর্ম ও মূলভাব লিখন) with reference citations.
              </p>
            </div>
          </div>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="title"
                rows={7}
                placeholder="পরের কারণে স্বার্থ দিয়া বলি,&#10;এ জীবন মন সকলি দাও,&#10;তার মত সুখ কোথাও কি আছে?&#10;আপনার কথা ভুলিয়া যাও।"
                {...register("title")}
