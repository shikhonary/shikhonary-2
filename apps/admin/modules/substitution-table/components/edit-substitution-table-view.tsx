...
import {
  useSubstitutionTableById,
  useUpdateSubstitutionTable,
...
import { Plus, Trash2, TableProperties, ArrowLeft, Loader2 } from "lucide-react"
import {
  Select,
  SelectContent,
  SelectItem,
...
const editSubstitutionTableFormSchema = z.object({
...
type EditSubstitutionTableFormData = z.infer<typeof editSubstitutionTableFormSchema>

export function EditSubstitutionTableView({ id }: { id: string }) {
  const router = useRouter()
  const { data: item, isLoading, isError } = useSubstitutionTableById(id)
  const updateMutation = useUpdateSubstitutionTable()
  ...
  } = useForm<EditSubstitutionTableFormData>({
    resolver: zodResolver(editSubstitutionTableFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      referenceText: "",
  ...
  // Prepopulate form when data is loaded
  useEffect(() => {
    if (item) {
      const classId = (item.subject as any)?.classSubjects?.[0]?.classId || ""
      reset({
        classId,
      ...
      const colA = item.columnA || []
      const colB = item.columnB || []
      const colC = item.columnC || []
      const maxRows = Math.max(colA.length, colB.length, colC.length, 1)

      const initialRows: Array<{ a: string; b: string; c: string }> = []
  ...
  const onSubmit = async (data: EditSubstitutionTableFormData) => {
    ...
    try {
      ...
      toast.success("Substitution Table updated successfully!")
      router.push("/substitution-tables")
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to update substitution table")
    }
  }
  ...
  if (isError || !item) {
    return (
      <div className="rounded-xl border border-error/30 bg-error/5 p-8 text-center text-error">
        <p className="font-bold text-base">Substitution Table not found</p>
        <Button asChild className="mt-4 bg-primary text-white text-xs font-bold rounded-lg cursor-pointer">
          <Link href="/substitution-tables">Back to Substitution Tables</Link>
        </Button>
      </div>
    )
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-outline">
              Modify the 3 columns (Column A, Column B, Column C) and metadata.
            </p>
          </div>
        </div>
      </div>
