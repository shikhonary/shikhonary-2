...
import { useImportThoughtExpansions, useAcademicClassesForSelection, useSubjectsForSelection } from "../services/use-thought-expansion"
...
export function repairJsonSyntax(raw: string): string {
  ...
  // 4. Fix unescaped raw newlines inside JSON string literals
  cleaned = cleaned.replace(/"((?:[^"\\]|\\.)*)"/gs, (match) => {
    return match.replace(/\r?\n/g, "\\n")
  })

  // 5. Remove trailing commas in objects & arrays (e.g. , ] -> ] and , } -> })
  cleaned = cleaned.replace(/,\s*([\]}])/g, "$1")

  // 6. Wrap single object in array if not already an array
      ...
      {isEditing ? (
        <div className="space-y-1.5">
          {multiline ? (
            <Textarea
              ...
              className={cn(
                "w-full rounded-lg border-2 border-primary bg-white p-2.5 text-sm font-medium focus:outline-hidden leading-relaxed",
                editValue && /[\u0980-\u09FF]/.test(editValue) && "font-solaiman text-base"
              )}
            />
          ) : (
            <Input
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <span className={cn(
              "flex-1 whitespace-pre-wrap font-medium text-on-surface text-sm",
              value && /[\u0980-\u09FF]/.test(value) ? "font-solaiman text-base leading-loose" : "leading-relaxed"
            )}>
              <RenderMath text={(value || placeholder)?.replace(/\\n/g, "\n")} isMath={isMath} />
            </span>
            <button
              type="button"
              onClick={(e) => {
...
interface EditableThoughtExpansionCardProps {
  index: number
  item: any
...
function EditableThoughtExpansionCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableThoughtExpansionCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2 border-l border-outline-variant/60 pl-2">
            <Button
              ...
              size="icon-xs"
              onClick={onDuplicate}
              title="Duplicate Thought Expansion"
      ...
      <div className="p-5 space-y-4">
        {/* Title / Couplet Text */}
        <div className="rounded-xl border border-secondary/20 bg-secondary-container/10 p-4 border-l-4 border-l-primary/70 shadow-2xs">
          <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-primary/80 mb-2">
            <span>📜 ভাব-সম্প্রসারণের মূলভাব / চরণ</span>
          </div>
          <EditableField
            value={item.title || ""}
            multiline
            placeholder="Enter proverb, maxim, or poetic couplet with line breaks..."
            textClassName="bg-transparent hover:bg-white/60 p-2"
...
export function ImportThoughtExpansionView() {
  const router = useRouter()
  const importMutation = useImportThoughtExpansions()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      setParseError(`Validation Error: ${err.message || "Could not validate Thought Expansion items."}`)
    }
  }
  ...
  const handleAddNewQuestionCard = () => {
    const newItem = {
      title: "New Proverb or Couplet Lines...",
      difficulty: "MEDIUM",
      popularityCount: 0,
      reference: [],
    }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ thoughtExpansions: payload })
      toast.success(`Successfully imported ${res.importedCount} Thought Expansions!`)
      setTimeout(() => {
        router.push("/thought-expansions")
      }, 1000)
    } catch (err: any) {
      toast.error(err.message || "Failed to bulk import thought expansions.")
    }
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Thought Expansions from JSON
          </h2>
          <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Bulk upload proverbs, maxims, or poetic couplets, or edit individual cards before importing into the question bank.
          </p>
        </div>
      ...
      <Card className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-xs">
        <CardHeader className="border-b border-outline-variant/30 p-6 bg-surface-container-low/30">
          ...
          <p className="font-body-md text-xs text-on-surface-variant mt-1">
            Select Academic Class and Subject. Thought expansion items will be assigned subject from the selection below.
          </p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-800 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {parsedItems.length} thought expansion item(s) parsed successfully.
                  </p>
                </div>
              </div>
              <Button
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-headline-md text-base font-bold text-on-surface flex items-center gap-2">
                    ...
                    Card Preview ({parsedItems.length} Thought Expansions)
                  </h4>
                </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/thought-expansions")}
            ...
            >
              ...
              <span>
                {importMutation.isPending
                  ? "Importing..."
                  : `Import ${parsedItems.length} Thought Expansions`}
              </span>
            </Button>
          </div>
        </CardContent>
