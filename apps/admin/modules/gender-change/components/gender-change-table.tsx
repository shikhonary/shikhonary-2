...
export interface GenderChangeItem {
  id: string
  word: string
  genderWord?: string | null
  difficulty: string
  reference: string[]
...
interface GenderChangeTableProps {
  items: GenderChangeItem[]
  isLoading: boolean
  isError: boolean
...
export function GenderChangeTable({
  items,
  isLoading,
  isError,
  onDelete,
      ...
      {isLoading && (
        <div className="py-16 text-center text-on-surface-variant rounded-xl border border-outline-variant bg-white">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <span className="font-body-md text-sm font-medium">Loading Gender Change Words...</span>
          </div>
        </div>
      )}
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          ...
                          {item.genderWord && (
                            <div className={cn(
                              ...
                              /[\u0980-\u09FF]/.test(item.genderWord) && "font-solaiman text-lg"
                            )}>
                              <span className="text-xs font-medium text-emerald-600">স্ত্রীবাচক:</span>
                              <RenderMath text={item.genderWord} isMath={true} />
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex md:flex-col justify-end items-center gap-2 shrink-0 border-t md:border-t-0 border-outline-variant/40 pt-3 md:pt-0 w-full md:w-auto">
                    <Link
                      href={`/gender-change/${item.id}/edit`}
