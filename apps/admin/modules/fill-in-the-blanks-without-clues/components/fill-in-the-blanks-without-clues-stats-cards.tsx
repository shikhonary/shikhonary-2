...
interface FillInTheBlanksWithoutCluesStatsCardsProps {
  totalCount?: number
  difficultyCounts?: {
    EASY: number
    MEDIUM: number
...
export function FillInTheBlanksWithoutCluesStatsCards({
  totalCount = 0,
  difficultyCounts = { EASY: 0, MEDIUM: 0, HARD: 0 },
  isLoading = false,
}: FillInTheBlanksWithoutCluesStatsCardsProps) {
  if (isLoading) {
    return (
      <div className="mb-6">
      ...
      <div className="hidden sm:grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 transition-all hover:shadow-sm">
          ...
          <div>
            ...
            <p className="font-headline-md text-2xl font-black text-on-surface">
        ...
        <div className="flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 transition-all hover:shadow-sm">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <p className="font-headline-md text-2xl font-black text-on-surface">
                {difficultyCounts.EASY.toLocaleString()}
              </p>
              <span className="font-label-sm text-xs font-semibold text-emerald-600">
                {totalCount > 0 ? Math.round((difficultyCounts.EASY / totalCount) * 100) : 0}%
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 transition-all hover:shadow-sm">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <p className="font-headline-md text-2xl font-black text-on-surface">
                {difficultyCounts.MEDIUM.toLocaleString()}
              </p>
              <span className="font-label-sm text-xs font-semibold text-amber-600">
                {totalCount > 0 ? Math.round((difficultyCounts.MEDIUM / totalCount) * 100) : 0}%
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 transition-all hover:shadow-sm">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <p className="font-headline-md text-2xl font-black text-on-surface">
                {difficultyCounts.HARD.toLocaleString()}
              </p>
              <span className="font-label-sm text-xs font-semibold text-red-600">
                {totalCount > 0 ? Math.round((difficultyCounts.HARD / totalCount) * 100) : 0}%
              </span>
            </div>
          </div>
        </div>
