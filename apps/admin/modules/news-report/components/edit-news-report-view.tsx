...
import { useNewsReportById, useUpdateNewsReport, useSubjectsForSelection, useAcademicClassesForSelection } from "../services/use-news-report"
...
const editNewsReportFormSchema = z.object({
...
type EditNewsReportFormData = z.infer<typeof editNewsReportFormSchema>

export function EditNewsReportView() {
  const router = useRouter()
  const params = useParams()
  const newsReportId = params.id as string

  const { data: newsReportData, isLoading: isNewsReportLoading, isError } = useNewsReportById(newsReportId)
  const updateMutation = useUpdateNewsReport()
  ...
  } = useForm<EditNewsReportFormData>({
    resolver: zodResolver(editNewsReportFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      title: "",
  ...
  useEffect(() => {
    if (newsReportData) {
      reset({
        classId: (newsReportData as any).subject?.classSubjects?.[0]?.classId || "",
        subjectId: newsReportData.subjectId,
        title: newsReportData.title,
        referenceText: newsReportData.reference?.join(", ") || "",
        difficulty: newsReportData.difficulty as any,
        popularityCount: String(newsReportData.popularityCount),
      })
    }
  }, [newsReportData, reset])
  ...
  const onSubmit = async (data: EditNewsReportFormData) => {
    ...
    try {
      ...
      toast.success("News report updated successfully.")
      setTimeout(() => {
        router.push("/news-reports")
      }, 500)
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to update News Report")
      toast.error(err.message || "Failed to update News Report")
    }
  }

  if (isNewsReportLoading) {
    return (
      ...
      <div className="mx-auto max-w-4xl py-12 text-center">
        <div className="rounded-2xl border border-dashed border-error/40 bg-error/5 p-12">
          ...
          <p className="mt-1 font-body-md text-sm text-outline">
            The news report you are trying to edit does not exist or has been removed.
          </p>
          <div className="mt-6">
            <Button asChild className="rounded-xl font-bold bg-primary text-white">
              <Link href="/news-reports">Back to News Reports</Link>
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-xs text-on-surface-variant">
                ID: <span className="font-mono text-[11px] text-outline">{newsReportId}</span>
              </p>
            </div>
          </div>
        </div>
