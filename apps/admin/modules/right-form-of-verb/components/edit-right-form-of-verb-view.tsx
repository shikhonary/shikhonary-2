...
import {
  useRightFormOfVerbById,
  useUpdateRightFormOfVerb,
...
const editRightFormOfVerbFormSchema = z.object({
...
type EditRightFormOfVerbFormData = z.infer<typeof editRightFormOfVerbFormSchema>
...
interface EditRightFormOfVerbViewProps {
  id: string
}

export function EditRightFormOfVerbView({ id }: EditRightFormOfVerbViewProps) {
  const router = useRouter()
  const { data: item, isLoading, isError } = useRightFormOfVerbById(id)
  const updateMutation = useUpdateRightFormOfVerb()
  ...
  } = useForm<EditRightFormOfVerbFormData>({
    resolver: zodResolver(editRightFormOfVerbFormSchema),
    defaultValues: {
      classId: "",
      subjectId: "",
      chapterId: "",
  ...
  const onSubmit = async (data: EditRightFormOfVerbFormData) => {
    try {
      ...
      toast.success("Right Form of Verbs question updated successfully")
      router.push("/right-form-of-verbs")
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to update question. Please check the form and try again.")
    }
  }

  if (isLoading) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-12 text-center text-xs text-outline">
  ...
  if (isError || !item) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-12 text-center text-xs text-error">
        Failed to load question with ID {id}.
      </div>
    )
  }
    ...
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
        <div>
          <h2 className="font-headline-md text-2xl font-extrabold text-primary sm:text-3xl">
            Edit Right Form of Verbs Question
          </h2>
          <p className="font-body-md text-xs text-on-surface-variant sm:text-sm">
            Update passage content, base verbs in brackets, references, and associations.
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <Button asChild variant="outline" disabled={isFormSubmitting || updateMutation.isPending}>
            <Link href="/right-form-of-verbs">Cancel</Link>
          </Button>
          <Button type="submit" disabled={isFormSubmitting || updateMutation.isPending}>
            {isFormSubmitting || updateMutation.isPending ? "Updating Question..." : "Update Question"}
          </Button>
        </div>
      </form>
    </div>
