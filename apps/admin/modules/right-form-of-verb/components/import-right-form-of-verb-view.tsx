...
const sampleJsonTemplate = `[
  {
    "content": "Fill in the blanks with the correct form of verbs given in the brackets:\\nAn ideal student always (learn) his lessons properly. He never (waste) his valuable time in idleness. He (know) that time once gone is gone forever. So, he (utilize) every moment in useful work. Thus he (succeed) in life.",
...
function RenderContentWithBracketHighlights({ text }: { text: string }) {
  if (!text) return null
      ...
      {isEditing ? (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            ...
            >
              {renderFormatted ? (
                <RenderContentWithBracketHighlights text={value || placeholder} />
              ) : (
                value || placeholder
              )}
            </span>
...
interface EditableRightFormOfVerbCardProps {
  index: number
  item: any
...
function EditableRightFormOfVerbCard({
  index,
  item,
  onChange,
  onDelete,
  onDuplicate,
}: EditableRightFormOfVerbCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-xl border border-outline-variant/60 bg-white shadow-xs transition-shadow hover:shadow-md">
      ...
      <div className="p-5 space-y-4">
        {/* Content with Bracket Highlights */}
        <EditableField
          label="Question / Passage Content with (Bracketed Verbs)"
          value={item.content || item.name || ""}
          multiline
          renderFormatted
          placeholder="Enter passage text with (verbs) in brackets..."
...
export function ImportRightFormOfVerbView() {
  const router = useRouter()
  const importMutation = useImportRightFormOfVerbs()
  ...
  const validateAndParseJson = (
    ...
    try {
      ...
      setParseError(
        `Validation Error: ${err.message || "Could not validate Right Form of Verbs items."}`
      )
    }
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importMutation.mutateAsync({ rightFormOfVerbs: payload })
      toast.success(`Successfully imported ${res.importedCount} Right Form of Verbs questions!`)
      setTimeout(() => {
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end justify-between">
        <div>
          ...
          <h2 className="mb-1.5 font-headline-md text-2xl sm:text-3xl font-extrabold text-primary">
            Import Right Form of Verbs from JSON
          </h2>
        ...
        <CardContent className="p-6 space-y-6">
          ...
          {parsedItems.length > 0 && (
            <div className="border-t border-outline-variant/30 pt-6 space-y-6">
              ...
              {/* Grid of Right Form of Verbs Preview Cards */}
              <div className="space-y-6">
                {parsedItems.map((item, idx) => (
                  <EditableRightFormOfVerbCard
                    key={idx}
                    index={idx}
                    item={item}
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-outline-variant pt-6">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/right-form-of-verbs")}
