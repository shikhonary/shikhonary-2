...
import { useDeleteAmplification, useBulkDeleteAmplifications } from "../services/use-amplification"
...
interface DeleteAmplificationModalProps {
  onSuccess?: () => void
}

export function DeleteAmplificationModal({ onSuccess }: DeleteAmplificationModalProps) {
  const { isOpen, amplificationId, amplificationName, selectedIds, closeModal } =
    useDeleteAmplificationModalStore()

  const deleteMutation = useDeleteAmplification()
  const bulkDeleteMutation = useBulkDeleteAmplifications()
  ...
  const handleConfirmDelete = async () => {
    try {
      if (isBulk) {
        ...
        toast.success(`Successfully deleted ${res.deletedCount} amplifications.`)
      } else if (amplificationId) {
        await deleteMutation.mutateAsync({ id: amplificationId })
        toast.success("Amplification deleted successfully.")
      }
      closeModal()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err.message || "Failed to delete Amplification")
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <DialogTitle className="font-headline-md text-lg font-bold tracking-tight text-on-surface normal-case">
              {isBulk ? `Delete ${selectedIds.length} Amplifications?` : "Delete Amplification?"}
            </DialogTitle>
            ...
            {isBulk ? (
              <>
                ...
                <span className="font-bold text-on-surface">
                  {selectedIds.length} selected amplifications
                </span>
                ? All associated records will be permanently removed.
              </>
            ) : (
              <>
                Are you sure you want to delete this amplification:
                <strong className="block mt-2 rounded-lg border border-outline-variant/40 bg-surface-container-lowest p-3 text-xs font-mono text-on-surface-variant line-clamp-2 break-all font-medium">
                  &quot;{amplificationName || "(No Name - ID: " + amplificationId + ")"}&quot;
                </strong>
              </>
            )}
          </DialogDescription>
          ...
          >
            {isDeleting ? (
              ...
              <>
                ...
                <span>Delete Amplification</span>
              </>
            )}
          </Button>
        </div>
